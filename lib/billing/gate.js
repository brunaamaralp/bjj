 ...
 * @returns {Promise<{ accessLevel: AccessLevel, status: string, needsPlan: boolean, currentPeriodEnd: string | null }>}
 */
export async function evaluateBillingAccess(storeId) {
  const sid = String(storeId || '').trim();
  if (!sid) {
    return { accessLevel: 'none', status: 'none', needsPlan: true, currentPeriodEnd: null };
  }
  if (!isBillingStoreConfigured()) {
    return { accessLevel: 'full', status: 'skipped', needsPlan: false, currentPeriodEnd: null };
  }
