 ...
 * @param {{ academyId: string, academyDoc: object, phones: string[], docsByPhone?: Record<string, object> }} params
 * `docsByPhone` evita re-query Appwrite quando o caller já tem o doc (ex.: GET thread).
 * @returns {Promise<{ avatars: Record<string, string>, persists: Array<{ docId: string, payload: object }> }>}
 */
export async function resolveInboxProfileAvatars({ academyId, academyDoc, phones, docsByPhone = null }) {
  ...
  const preloaded =
    docsByPhone && typeof docsByPhone === 'object' && !Array.isArray(docsByPhone) ? docsByPhone : {};
  ...
  for (const phone of unique) {
    const doc = preloaded[phone] ?? (await findConversationDoc(phone, aid));
