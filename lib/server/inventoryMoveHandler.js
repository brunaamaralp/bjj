...
import { buildAvailableQuantityPatch } from '../../src/lib/dualStockPools.js';
import { buildInventoryMoveStockPatch } from '../../src/lib/inventoryStockPatch.js';
...
export async function syncConsolidatedRestockForAcademy(databases, dbId, academyId, stockItemsCol) {
  ...
  const description = taskDescriptionForAppwrite(buildConsolidatedRestockTaskDescription(critical));
  const ts = nowIso();
  const payload = {
...
/** Payload de despesa no Caixa para entrada de estoque (testável). */
export function buildStockPurchaseFinancePayload({
  academyId,
  purchasePrice,
  itemName,
  quantity,
  ...
  const moveId = String(stockMoveId || '').trim();
  ...
  return {
    ...
    origin_id: moveId.slice(0, 64),
  };
}

export async function patchStockMoveFinancialTxId(databases, dbId, stockMovesCol, moveId, financialTxId) {
  if (!moveId || !financialTxId || !stockMovesCol) return;
  try {
    await databases.updateDocument(dbId, stockMovesCol, moveId, {
      financial_tx_id: String(financialTxId).slice(0, 64),
    });
  } catch (e) {
    ...
    if (/unknown attribute/i.test(msg)) {
      console.warn(
        JSON.stringify({
          event: 'stock_move_financial_tx_id_attr_missing',
          move_id: moveId,
...
export async function createStockPurchaseFinanceTx(databases, dbId, academyDoc, opts) {
  return maybeCreateFinanceExpense(databases, dbId, academyDoc, opts);
}
...
async function maybeCreateFinanceExpense(databases, dbId, academyDoc, {
  ...
  try {
    const doc = await databases.createDocument(dbId, FINANCIAL_TX_COL, ID.unique(), forDb, [
...
export async function executeInventoryMove(databases, opts) {
  ...
  const parentType = resolved.parent?.type || item.type;

  if (tipo === 'avulso') {
    itemUpdates.status_par = status_par || item.status_par || 'completo';
  } else {
    Object.assign(itemUpdates, buildInventoryMoveStockPatch(item, parentType, tipo, quantidade));
  }
  ...
  if (payment_method) {
    movePayload.payment_method = String(payment_method).trim().slice(0, 32);
  }
  ...
  if (tipo === 'entrada' && purchase_price != null && academyDoc && academyId) {
    ...
    if (financialTxId) {
      await patchStockMoveFinancialTxId(databases, dbId, stockMovesCol, move.$id, financialTxId);
      console.warn(
        JSON.stringify({
          event: 'stock_entry_finance_linked',
          academy_id: academyId,
          move_id: move.$id,
          financial_tx_id: financialTxId,
        })
  ...
  if (
    ...
    try {
      const borrower =
        kimonoLoan && typeof kimonoLoan === 'object'
          ? {
              borrower_type: kimonoLoan.borrower_type || kimonoLoan.borrowerType,
              borrower_id: kimonoLoan.borrower_id || kimonoLoan.borrowerId,
              borrower_name: kimonoLoan.borrower_name || kimonoLoan.borrowerName,
            }
          : null;
      await recordKimonoLoanAfterRentalExit(databases, {
        ...
        academyId,
        variantId: itemEstoqueId,
        resolved,
        quantity: Math.abs(Math.trunc(quantidade)),
        stockMoveOutId: move.$id,
        lentAt: move.$createdAt,
        borrower,
        lentByUserId: usuario_id,
        notes: kimonoLoan?.notes || motivo,
        source: kimonoLoan?.source || 'inventory',
        move,
      });
    } catch (e) {
      console.warn('[inventory-move] kimono_loan_record:', e?.message || e);
    }
  }
...
export async function executeInventoryAdjustment(databases, opts) {
  ...
  const updated = await databases.updateDocument(dbId, stockCol, itemId, {
    ...buildAvailableQuantityPatch(item, parentType, nextQty),
    last_updated: ts,
  });
  ...
  if (academyId && stockItemsCol) {
    try {
      ...
      console.warn('[inventory] sync restock task after adjust:', syncErr?.message || syncErr);
    }
  }
