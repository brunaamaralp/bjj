...
async function findTrocoTxForPayment(paymentId) {
  if (!FINANCIAL_TX_COL || !paymentId) return null;
  try {
...
async function mirrorStudentPaymentTroco({ paymentDoc, data }) {
  ...
  if (!FINANCIAL_TX_COL || !paymentId) return { warning: null };

  const existing = await findTrocoTxForPayment(paymentId);
  if (troco <= 0) {
    if (existing?.$id) {
      try {
        await databases.updateDocument(DB_ID, FINANCIAL_TX_COL, existing.$id, { status: 'cancelled' });
      } catch (e) {
  ...
  try {
    if (existing?.$id) {
      await writeFinancialTx(
        (doc) => databases.updateDocument(DB_ID, FINANCIAL_TX_COL, existing.$id, doc),
        trocoPayload
      );
    } else {
