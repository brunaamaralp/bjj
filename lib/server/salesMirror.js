...
/** Prefixo da nota do TX pendente de saldo (entrada parcial). */
export const SALE_BALANCE_NOTE_PREFIX = 'Saldo a receber';

export function isSaleBalancePendingTx(doc) {
  if (!doc) return false;
  if (String(doc.status || '').toLowerCase() !== 'pending') return false;
  const note = String(doc.note || doc.planName || '');
  return note.startsWith(SALE_BALANCE_NOTE_PREFIX);
}
...
/** Entrada parcial: espelha o recebido agora + TX pendente do saldo restante. */
export async function mirrorPartialSale({
  vendaId,
  totalRounded,
  paidAmount,
  pagamentosNorm,
  ...
  const paidAmountRounded = roundMoney(paidAmount);
  const balanceGross = roundMoney(totalRounded - paidAmountRounded);
  const clienteLabel = String(clienteNome || 'cliente avulso').trim() || 'cliente avulso';
  const balanceNote = `${SALE_BALANCE_NOTE_PREFIX} — ${clienteLabel}`;

  if (pagamentosNorm?.length) {
    const mixed = await mirrorMixedPayments({
      vendaId,
      academyId,
      aluno_id,
      pagamentosNorm,
  ...
  if (balanceGross <= 0.009) return { warnings };

  const saleGross = roundMoney(lineKindGross.VENDA_PRODUTO || 0);
  const rentalGross = roundMoney(lineKindGross.ALUGUEL_RECEITA || 0);
  const totalLineGross = roundMoney(saleGross + rentalGross);
  const entries = [];

  if (totalLineGross > 0.009 && rentalGross > 0.009 && saleGross > 0.009) {
    const saleShare = roundMoney((balanceGross * saleGross) / totalLineGross);
    const rentalShare = roundMoney(balanceGross - saleShare);
    if (saleShare > 0.009) entries.push({ categoryKey: 'VENDA_PRODUTO', gross: saleShare });
    if (rentalShare > 0.009) entries.push({ categoryKey: 'ALUGUEL_RECEITA', gross: rentalShare });
  } else if (rentalGross > 0.009 && saleGross <= 0.009) {
    entries.push({ categoryKey: 'ALUGUEL_RECEITA', gross: balanceGross });
  } else {
    entries.push({ categoryKey: 'VENDA_PRODUTO', gross: balanceGross });
  }
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      console.warn('[salesMirror] partial balance', entry.categoryKey, e?.message);
      warnings.push(`Saldo pendente (${cat.label}) não criado no caixa.`);
    }
  }
