...
import { tryAcquireAgentLock, releaseAgentLock } from './agentProcessingLock.js';
...
export default async function handler(req, res) {
  ...
  if (isWhatsAppGroupId(phone)) {
    return res.status(200).json({ sent: false, skipped: true, reason: 'whatsapp_group' });
  }
  ...
  try {
    ...
    if (!academyDoc) {
      ...
      return res.status(403).json({ error: 'Academia inv\u00e1lida ou inativa' });
    }
    ...
    try {
      ...
      if (e instanceof AiFeatureDisabledError) {
        return res.status(403).json({ sent: false, error: e.code, message: e.message });
      }
      throw e;
    }
    if (instOut) {
      ...
      if (!zid || zid !== instOut) {
        console.error('[agent] instanceId n\u00e3o pertence \u00e0 academia', { academyId: academy, instanceId: instOut });
        return res.status(403).json({ error: 'Inst\u00e2ncia n\u00e3o pertence a esta academia' });
      }
    }
    ...
    let convForLock = convForQuota;
    if (!convForLock?.$id) {
      convForLock = await getOrCreateConversationDoc(phone, academy, academyDoc).catch(() => null);
    }
    convIdForLock = String(convForLock?.$id || inboundId || '').trim();
    if (!convIdForLock) {
      console.error('[agent/process] conversa não encontrada para lock', { requestId, phone, academy });
      return res.status(200).json({ sent: false, error: 'no_conversation' });
    }

    const lockResult = await tryAcquireAgentLock(convIdForLock);
    if (!lockResult.acquired) {
      console.log(`[AgentProcess] Lock ativo para conv ${convIdForLock} — ignorando mensagem duplicada`, {
        requestId,
    ...
    try {
      ...
      let sent = await sendZapsterText({ recipient: phone, text: resposta, instanceId: instOut });
      if (!sent?.ok) {
        await new Promise((r) => setTimeout(r, 400));
        sent = await sendZapsterText({ recipient: phone, text: resposta, instanceId: instOut });
      }
      if (!sent?.ok) {
        let convIdForLog = '';
        try {
          const convLog = await findConversationDoc(phone, academy);
          convIdForLog = String(convLog?.$id || '').trim();
        } catch {
          void 0;
        }
        logStructured('agent_send_failed', {
          event: 'agent_send_failed',
          academy_id: academy,
          phone,
          conversation_id: convIdForLog || null,
          error: String(sent?.erro || 'unknown').slice(0, 500),
        ...
        void createInternalNotification({
          ...
          title: 'Falha no envio do assistente',
          body: `O assistente gerou uma resposta mas não conseguiu enviar para ${phone}. Verifique a conexão com o WhatsApp.`,
          action_url: `/inbox?phone=${encodeURIComponent(phone)}`,
          severity: 'high',
          phone,
      ...
      logApiError('agent/process', e);
      return res.status(200).json({ sent: false, error: apiErro(e, 'send') });
    }
  } finally {
    if (lockAcquired && convIdForLock) {
      await releaseAgentLock(convIdForLock);
    }
  }
}
