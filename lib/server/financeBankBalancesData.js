...
export function computeBankBalancesPayloadFromSettledDocs(rawDocs, asOfYmd, financeConfig) {
...
export async function fetchAllSettledTx(academyId, asOfYmd, opts = {}) {
  ...
  const startIso = fromYmd ? new Date(`${fromYmd}T00:00:00`).toISOString() : null;
  let all = [];
  let cursor = null;
