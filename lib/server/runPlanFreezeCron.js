...
function mapDocToStudentLike(doc, todayYmd) {
  return {
    ...
    plan: doc.plan,
    plan_billing: doc.plan_billing || doc.planBilling,
  };
}
...
async function maybeAlertFreezeLimit(databases, dbId, doc, todayYmd) {
  ...
  const maxDays = freezeQuotaMaxDays(studentLike) || 90;
  const alertAt = freezeLimitAlertDays(studentLike);
  const remainingAtAlert = Math.max(0, maxDays - alertAt);
    ...
    try {
      await databases.createDocument(
        ...
        ID.unique(),
        {
          ...
          description: `${description}\n---\nAluno com ${alertAt}+ dias de trancamento no ciclo do plano (restam ≤${remainingAtAlert} dias da cota de ${maxDays}).`,
          status: 'pending',
          due_date: todayYmd,
          assigned_to: '',
          lead_id: leadId,
  ...
  await addLeadEventServer({
    ...
    text: `Aluno próximo do limite de trancamento (${projected}/${maxDays} dias no ciclo do plano).`,
    payloadJson: { projected_days: projected, freeze_start: startYmd, quota_max: maxDays },
    createdBy: 'cron',
  });
...
export async function runPlanFreezeCron(databases, dbId, leadsCol, academiesCol) {
  ...
  for (const doc of docs) {
    ...
    try {
      ...
      const quotaYear = studentFreezeCycleStartYmd(studentLike, null, new Date(`${today}T12:00:00`));
