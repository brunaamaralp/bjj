...
import { totalPhysicalQuantity } from '../../src/lib/dualStockPools.js';
import {
...
import {
  countSalesLinkedToStockItem,
  hasBlockingStockMovesForItem,
} from './productDeletePolicy.js';
import {
  isProductImageStorageConfigured,
  isAllowedProductImageMime,
  MAX_PRODUCT_IMAGE_BYTES,
  storeProductImageBuffer,
...
async function countSaleItemsForProduct(itemId, academyId, legacyStockItemId = '') {
  return countSalesLinkedToStockItem(databases, {
    dbId: DB_ID,
...
async function hasStockMovesForItem(itemId) {
  return hasBlockingStockMovesForItem(
    databases,
    DB_ID,
    STOCK_MOVES_COL,
    STOCK_ITEMS_COL,
...
async function checkParentProductDelete(databases, dbId, academyId, productId) {
  ...
  for (const v of variants) {
    const vid = v.$id;
    const legacyId = String(v.legacy_stock_item_id || '').trim();
    const saleCount = await countSaleItemsForProduct(vid, academyId, legacyId);
    if (saleCount > 0) has_sales = true;
    if (await hasStockMovesForItem(vid)) has_stock_moves = true;
    const physical = totalPhysicalQuantity(v);
    total_quantity += physical != null ? physical : Number(v.current_quantity) || 0;
  }

  return {
    has_sales,
    has_stock_moves,
    can_delete: !has_sales,
...
async function deleteParentProduct(databases, dbId, academyId, productId) {
  ...
  for (const v of variants) {
    ...
    const legacyId = String(v.legacy_stock_item_id || '').trim();
    if (legacyId && legacyId !== v.$id) {
      await deleteStockMovesForItem(legacyId);
    }
  ...
  if (method === 'POST') {
    ...
    if (action === 'upload_image') {
      if (!isProductImageStorageConfigured()) {
        return json(res, 503, {
          sucesso: false,
          erro: 'Upload de imagem não configurado no servidor.',
        });
      }

      const raw = String(req.body.image_base64 || req.body.data || '').trim();
      if (!raw) return json(res, 400, { sucesso: false, erro: 'image_base64 obrigatório' });

      const mimeType = String(req.body.mime_type || req.body.mimeType || '').trim().toLowerCase();
      if (!isAllowedProductImageMime(mimeType)) {
        return json(res, 400, { sucesso: false, erro: 'Use JPG, PNG ou WebP.' });
      }
      ...
      try {
        ...
        return json(res, 400, { sucesso: false, erro: 'Imagem inválida.' });
      }

      if (!buffer.length) return json(res, 400, { sucesso: false, erro: 'Arquivo vazio.' });
      if (buffer.length > MAX_PRODUCT_IMAGE_BYTES) {
        return json(res, 400, {
          sucesso: false,
          erro: `Imagem muito grande. Máximo: ${Math.round(MAX_PRODUCT_IMAGE_BYTES / (1024 * 1024))} MB.`,
        });
      }

      const filename = String(req.body.filename || 'produto.jpg').trim().slice(0, 128) || 'produto.jpg';
      try {
        const stored = await storeProductImageBuffer(buffer, { mimeType, filename });
        return json(res, 200, {
          sucesso: true,
        ...
        console.error('[products] upload_image:', e);
        const msg = String(e?.message || e || '');
        if (/not found|404|bucket/i.test(msg)) {
          return json(res, 404, {
            sucesso: false,
            erro:
              'Bucket de imagens de produto não encontrado. Rode npm run provision:product-images-bucket.',
          });
        }
        if (msg === 'too_large') {
          return json(res, 400, { sucesso: false, erro: 'Imagem muito grande.' });
        }
      ...
      try {
        ...
        for (const group of groups) {
          ...
          const out = await createProductWithVariants(databases, {
            ...
            me,
            body,
            reuseExistingParent: true,
          });
          if (out.error) {
            errors.push({ nome: body.nome, erro: out.error });
            continue;
          }
          if (out.parent_created) parentsCreated += 1;
    ...
    if (action === 'delete_product' && isParentVariantCatalogEnabled()) {
      ...
      try {
        ...
        return json(res, 500, { sucesso: false, erro: apiErro(e, 'delete') });
      }
    }
    ...
    if (action === 'check_delete') {
      ...
      const legacyId = String(resolved.doc.legacy_stock_item_id || '').trim();
      const totalSales = await countSaleItemsForProduct(itemId, academyId, legacyId);
      const hasStockMoves = await hasStockMovesForItem(itemId);
      const product =
    ...
    if (action === 'delete') {
      ...
      const legacyId = String(resolved.doc.legacy_stock_item_id || '').trim();
      const saleCount = await countSaleItemsForProduct(itemId, academyId, legacyId);
      if (saleCount > 0) {
        return json(res, 409, {
          sucesso: false,
          has_sales: true,
