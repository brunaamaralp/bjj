...
async function checkParentProductDelete(databases, dbId, academyId, productId) {
  ...
  return {
    has_sales,
    has_stock_moves,
    can_delete: !has_sales && !has_stock_moves,
...
async function deleteParentProduct(databases, dbId, academyId, productId) {
  ...
  if (check.has_stock_moves) {
    return {
      error:
        'Produto com movimentações de estoque não pode ser excluído. Desative-o ou remova ajustes/vendas antes.',
      status: 409,
  ...
  if (method === 'POST') {
    ...
    if (action === 'check_delete') {
      ...
      return json(res, 200, {
        ...
        can_delete: totalSales === 0 && !hasStockMoves,
    ...
    if (action === 'delete') {
      ...
      if (await hasStockMovesForItem(itemId)) {
        return json(res, 409, {
          sucesso: false,
          has_stock_moves: true,
          erro:
