...
import { mapStockProductDoc, buildProductPayloadFromBody } from './stockProductMap.js';
...
export default async function productsHandler(req, res) {
  ...
  if (method === 'POST') {
    ...
    if (action === 'create') {
      ...
      try {
        const doc = await databases.createDocument(
          ...
          STOCK_ITEMS_COL,
          ID.unique(),
          { ...built.payload, academy_id: academyId },
    ...
    if (action === 'update') {
      ...
      try {
        const updated = await databases.updateDocument(DB_ID, STOCK_ITEMS_COL, itemId, built.payload);
