 ...
 * POST /api/students/freeze | /api/students/unfreeze | /api/students/deactivate
...
import { executeFreezeServer, executeUnfreezeServer } from './planFreezeExecute.js';
...
async function handleUnfreeze(req, res, academyId, me, academyDoc) {
  let body = req.body;
  if (typeof body === 'string') {
    try {
      body = JSON.parse(body);
  ...
  const studentId = String(body.student_id || body.lead_id || '').trim();
  const early = body.early !== false;

  if (!studentId) return json(res, 400, { sucesso: false, erro: 'student_id_required' });

  try {
    assertRoleOwner(academyDoc, me.$id);
    const result = await executeUnfreezeServer({
      databases,
      dbId: DB_ID,
      studentsCol: STUDENTS_COL,
      planFreezesCol: PLAN_FREEZES_COL,
    ...
    if (!result.ok) {
      if (result.error === 'student_not_frozen') {
        return json(res, 400, { sucesso: false, erro: 'Aluno não está com trancamento ativo.' });
      }
      return json(res, 400, { sucesso: false, erro: result.error });
    }
    return json(res, 200, {
      sucesso: true,
      actualEndYmd: result.actualEndYmd,
      daysCharged: result.daysCharged,
      freeze_days_used: result.freeze_days_used,
      freeze_quota_year: result.freeze_quota_year,
    ...
    if (e.code === 'FORBIDDEN') {
      return json(res, 403, {
        sucesso: false,
        erro: e.message === 'owner_required' ? 'Apenas o titular da academia pode encerrar trancamentos' : 'Acesso negado',
      });
    }
    console.error('[students/unfreeze]', studentId, e?.message || e);
    return json(res, 500, { sucesso: false, erro: apiErro(e, 'action') });
  }
}
...
export default async function studentsHandler(req, res) {
  ...
  if (action === 'unfreeze' && req.method === 'POST') {
    return handleUnfreeze(req, res, academyId, me, academyDoc);
  }
