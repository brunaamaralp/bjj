...
async function handleFreeze(req, res, academyId, me, academyDoc) {
  ...
  try {
    ...
    await databases.updateDocument(DB_ID, STUDENTS_COL, studentId, studentPatch);
    ...
    if (PLAN_FREEZES_COL) {
      ...
      try {
        await databases.createDocument(DB_ID, PLAN_FREEZES_COL, ID.unique(), freezeDoc);
      } catch (freezeErr) {
        await databases.updateDocument(DB_ID, STUDENTS_COL, studentId, {
        ...
        if (/end_date|days|required|schema_incompatible/i.test(freezeMsg)) {
          throw new Error(
            'Schema plan_freezes desatualizado. Execute npm run provision:plan-freeze e tente novamente.'
          );
        }
        throw freezeErr;
      }
