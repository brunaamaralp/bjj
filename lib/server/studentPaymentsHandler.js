...
function buildPayload(data, financeConfig, studentDoc) {
  ...
  if (!Number.isFinite(expected) || expected <= 0) {
    if (category === PAYMENT_CATEGORY.FEE || category === PAYMENT_CATEGORY.OTHER) {
      expected = Number(data.amount ?? data.paid_amount) || 0;
    } else {
...
function preserveExistingLaunchAmounts(prev, payload) {
  ...
  if (Number.isFinite(prevExpected) && prevExpected >= 0) {
...
export async function handleCreateStudentPayment(req, res, academyId, me, academyDoc) {
  ...
  try {
    ...
    if (prev) {
      const nextPayload = preserveExistingLaunchAmounts(prev, payload);
