...
export async function handleCreateStudentPayment(req, res, academyId, me, academyDoc) {
  ...
  try {
    ...
    return json(res, 200, { ok: true, payment: doc });
  } catch (e) {
...
export async function handlePatchStudentPayment(req, res, academyId, me) {
  ...
  try {
    ...
    const patch = { ...body };
    delete patch.action;
    if (isReverse) patch.status = 'cancelled';
...
export default async function studentPaymentsHandler(req, res) {
  ...
  if (req.method === 'PATCH') {
    return handlePatchStudentPayment(req, res, academyId, me);
  }
