  ...
  try {
    switch (eventType) {
      case 'subscription.payment.paid': {
        ...
        const referenceMonth = billingCtx.referenceMonth;
        ...
        const paymentDoc = upsertResult.doc || {};
        let mirrorResult = { mirrorId: paymentDoc.financial_tx_id || null };
        ...
        if (!skipMirror) {
          mirrorResult = await mirrorStudentPaymentToFinancialTx({
            paymentDoc: {
              ...
              lead_id: studentId,
              amount: amountReais,
              paid_amount: Number(paymentDoc.paid_amount ?? amountReais),
              status: 'paid',
              paid_at: paymentDoc.paid_at || paidAtIso,
              reference_month: paymentDoc.reference_month || referenceMonth,
              method: paymentDoc.method || 'pagbank',
              payment_category: paymentDoc.payment_category || 'plan',
              financial_tx_id: paymentDoc.financial_tx_id || '',
              gateway_payment_id: paymentDoc.gateway_payment_id || paymentId || '',
              gateway_provider: paymentDoc.gateway_provider || 'pagbank',
            },
            payload: {
              academy_id: academyId,
              lead_id: studentId,
            ...
            financeConfig,
            studentDoc,
            existingTxId: paymentDoc.financial_tx_id || null,
          });
        }
        ...
        if (paymentDocId) {
          ...
          await databases.updateDocument(DB_ID, PAGBANK_PAYMENTS_COL, paymentDocId, pagbankPatch);
          if (mirrorResult?.mirrorId) {
