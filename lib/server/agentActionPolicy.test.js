...
describe('agentActionPolicy', () => {
  ...
  it('conversation timeline enabled by default', () => {
    expect(isConversationTimelineEnabled({})).toBe(true);
  });

  it('respects conversation_timeline disable', () => {
    expect(
      isConversationTimelineEnabled({
        modules: JSON.stringify({
          ai_actions: { enabled: true, conversation_timeline: { enabled: false } },
        }),
      })
    ).toBe(false);
  });
