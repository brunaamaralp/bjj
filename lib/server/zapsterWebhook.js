...
function checkWebhookToken(req, res) {
  ...
  if (!expectedToken) {
    ...
    console.log('[debug] webhook 401: ZAPSTER_WEBHOOK_TOKEN ausente ou vazio no servidor');
  ...
  if (!safeCompare(provided, expectedToken)) {
    ...
    console.log('[debug] webhook 401: token do request n\u00e3o confere com ZAPSTER_WEBHOOK_TOKEN (Zapster vs Vercel)');
...
/** Áudio ou nota de voz (ptt) no webhook Zapster — mesma forma `content.media.url` que imagens. */
...
async function resolveAcademyIdForWebhook(req, instanceId) {
  ...
  if (byQuery) {
    console.warn('[zapster][webhook][debug] fallback academyId via query', {
      academyId: byQuery,
...
async function saveInboundMessage({
  ...
  if (nextName) {
    try {
      ...
      const shouldFillContactName = !existingContactName || existingSource !== 'manual';
      const payload = shouldFillContactName
        ? {
            ...basePayload,
      ...
      console.warn('[zapster][webhook] não foi possível persistir nome de contato', {
        phone,
        academyId,
        erro: e?.message || String(e)
      });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const academyId = await resolveAcademyIdForWebhook(req, instanceId);
    if (!academyId) {
      console.warn('[zapster][webhook][debug] instance_not_mapped', {
        event: String(event || '').trim(),
        instanceId: instanceId || null,
