...
import { ensureWhatsAppInboundLead } from './ensureWhatsAppInboundLead.js';
...
async function extractIncomingImageMedia(msg, { academyId, messageId } = {}) {
...
async function extractIncomingAudioMedia(msg, { academyId, messageId } = {}) {
  const topType = String(msg?.type || '').toLowerCase();
  const content = msg?.content && typeof msg.content === 'object' ? msg.content : {};
...
async function inferInboundAudioFromMessage(msg, text, { academyId, messageId } = {}) {
  ...
  if (!mediaUrl) return { messageType: 'text', mediaUrl: null, storageFileId: null, media_stored: false, mimeType: null };
  const t = String(text || '').trim();
  ...
  return { messageType: 'text', mediaUrl: null, storageFileId: null, media_stored: false, mimeType: null };
}
...
function inboundMediaFieldsFromExtras(mt, extras = {}) {
  ...
  else if (type === 'image' || type === 'audio') out.mediaUrl = null;
  return out;
}
...
function pickReadEventContactPhone(msg, body, waPhoneDigits) {
  ...
  if (senderPhone && wa && senderPhone !== wa && !isGroupId(senderPhone)) {
    return senderPhone;
  }
  if (recipientPhone && wa && recipientPhone !== wa && !isGroupId(recipientPhone)) {
    return recipientPhone;
  }
  if (senderPhone && !isGroupId(senderPhone)) return senderPhone;
  if (recipientPhone && !isGroupId(recipientPhone)) return recipientPhone;
  return '';
}
...
async function handleMessageSentWebhook(req, res, body, msg) {
  ...
  if ((sentType === 'image' || sentType === 'audio' || sentType === 'ptt') && !sentMediaUrl) {
    const content =
      sentType === 'image' ? '[Imagem enviada pelo celular]' : '[Áudio enviado pelo celular]';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const imageMedia = await extractIncomingImageMedia(msg, { academyId, messageId });
    if (imageMedia) {
      const displayContent = String(imageMedia.caption || '').trim() || '[imagem]';
      const saved = await saveInboundMessage({
        ...
        messageId,
        messageType: 'image',
        mediaUrl: imageMedia.mediaUrl,
        storageFileId: imageMedia.storageFileId,
        media_stored: imageMedia.media_stored,
    ...
    const audioMedia = await extractIncomingAudioMedia(msg, { academyId, messageId });
    if (audioMedia) {
      const displayContent = String(audioMedia.caption || '').trim() || '🎵 [Áudio recebido]';
      const saved = await saveInboundMessage({
        academyId,
        academyDoc,
        phone,
        ...
        messageId,
        messageType: 'audio',
        mediaUrl: audioMedia.mediaUrl,
        storageFileId: audioMedia.storageFileId,
        media_stored: audioMedia.media_stored,
