/**
 * PATCH /api/sales — liquida venda pendente ou parcial (receber depois / pagamento parcial).
 */
...
import { mirrorMixedPayments, refreshPendingSaleBalance } from './salesMirror.js';
...
function saleDescriptionFromDoc(saleDoc) {
...
export default async function salesLiquidateHandler(req, res) {
  ...
  if (status !== 'pendente' && status !== 'parcial') {
    return json(res, 409, { ok: false, error: 'sale_not_open' });
  }

  const pagamentosSnapshot = String(body?.pagamentos_snapshot ?? '').trim();
  const currentPagamentos = String(saleDoc.pagamentos_json || '[]').trim();
  if (pagamentosSnapshot && pagamentosSnapshot !== currentPagamentos) {
    return json(res, 409, { ok: false, error: 'sale_payment_stale' });
  }

  const totalRounded = roundMoney(Number(saleDoc.total) || 0);
  const alreadyPaid = salePaidAmountNet(saleDoc.pagamentos_json);
  ...
  const check = validatePagamentosForSettlement(pagamentosNorm, totalRounded, {
    allowPartial: true,
    alreadyPaid,
  });
  if (!check.ok) {
    return json(res, 400, {
      ok: false,
      error: check.reason === 'exceeds_remaining' ? 'pagamentos_exceed_remaining' : 'pagamentos_total_mismatch',
      expected: totalRounded,
      received: check.net,
      already_paid: alreadyPaid,
      remaining: check.remaining ?? saleRemainingAmount(totalRounded, alreadyPaid),
    });
  }

  const mergedPagamentos = mergePagamentosLists(parsePagamentosJson(saleDoc.pagamentos_json), pagamentosNorm);
  ...
  const newStatus = check.isComplete ? 'concluida' : 'parcial';
  const description = saleDescriptionFromDoc(saleDoc);
  ...
  try {
    ...
    const next = { status: newStatus, forma_pagamento: formaFinal };
  ...
  if (FINANCIAL_TX_COL) {
    const pendingTxs = (await listSaleFinancialTx(vendaId)).filter(
      (d) => String(d.status || '').toLowerCase() === 'pending'
    );
    const useSinglePendingSettle =
      check.isComplete &&
      pendingTxs.length === 1 &&
      pagamentosNorm.length === 1 &&
      alreadyPaid <= 0.009;

    if (useSinglePendingSettle) {
      const p = pagamentosNorm[0];
      ...
      if (!check.isComplete) {
        await refreshPendingSaleBalance({
          ...
          academyId,
          aluno_id: saleDoc.aluno_id,
          remainingGross: check.remaining,
          description,
          due_date: saleDoc.due_date,
        });
      } else {
  ...
  return json(res, 200, {
    ...
    venda_id: vendaId,
    total: totalRounded,
    paid_amount: check.newPaid,
    remaining: check.remaining,
    status: newStatus,
    partial: !check.isComplete,
  });
}