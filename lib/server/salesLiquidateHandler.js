...
export default async function salesLiquidateHandler(req, res) {
  ...
  const isPartialSale = status === 'parcial';
  const totalRounded = roundMoney(Number(saleDoc.total) || 0);
  const paidSoFar = roundMoney(Number(saleDoc.paid_amount) || 0);
  const balanceDue = isPartialSale ? roundMoney(totalRounded - paidSoFar) : totalRounded;
