/**
 * PATCH /api/sales — liquida venda pendente (receber depois).
 */
...
import { mirrorMixedPayments } from './salesMirror.js';
...
export default async function salesLiquidateHandler(req, res) {
  ...
  const formaFinal = buildFormaPagamentoResumo(pagamentosNorm);
  const pagamentosJson = JSON.stringify(pagamentosNorm);
  ...
  try {
    ...
    const next = { status: 'concluida', forma_pagamento: formaFinal };
  ...
  if (FINANCIAL_TX_COL && pendingTxs.length === 1 && pagamentosNorm.length === 1) {
    ...
    const bankAccount = resolveBankAccountForPayment('', financeConfig);
  ...
  } else if (FINANCIAL_TX_COL && pendingTxs.length) {
