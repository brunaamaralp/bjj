...
function buildMessage(type, academy, data = {}) {
  ...
  switch (type) {
    ...
    case 'sale_mirror_failed':
      return [
        `⚠️ *${name}* — venda sem espelho no Caixa`,
        ``,
        `A venda *${data.venda_short || data.venda_id || '—'}* foi concluída, mas o lançamento financeiro não foi criado automaticamente.`,
        data.warnings ? `\n${data.warnings}` : '',
        ``,
        `Revise o Caixa e registre manualmente se necessário.`,
      ].join('\n');
