 ...
 * Troca o produto de uma linha em venda concluída (estoque + total + caixa).
 */
...
import { parsePagamentosJson, roundMoney } from './salePayments.js';
...
async function syncFinanceAfterItemChange({
  ...
  const revenueTxs = txs.filter((d) => {
    ...
    return origin === 'sale' && type !== 'stock_purchase' && type !== 'refund';
  });
...
export default async function salesUpdateItemHandler(req, res) {
  ...
  const isOwner = await isAcademyOwnerOrAdminUser(academyDoc, me.$id);
  if (!isOwner) {
  ...
  const saleAcademy = String(saleDoc.academyId || saleDoc.academy_id || '').trim();
  if (saleAcademy && saleAcademy !== academyId) {
    return json(res, 403, { ok: false, error: 'forbidden' });
  }
