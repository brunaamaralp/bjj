...
export async function listAcademyStockMoves(databases, opts) {
  ...
  const uniqueItemIds = [
    ...new Set(docs.map((d) => String(d.item_estoque_id || '').trim()).filter(Boolean)),
  ];

  itemLabelCache.clear();
  const [, txStatusById] = await Promise.all([
    Promise.all(
      uniqueItemIds.map((itemId) =>
        resolveItemLabel(databases, dbId, stockItemsCol, itemId, academyId)
      )
    ),
    fetchFinancialTxStatusMap(databases, dbId, txIds, academyId),
  ]);

  const moves = docs.map((doc) => {
    const itemId = String(doc.item_estoque_id || '').trim();
    const itemLabel = itemId
      ? itemLabelCache.get(`${academyId}:${itemId}`) || '—'
      : '—';
    const txId = String(doc.financial_tx_id || '').trim();
    return mapStockMoveRow(doc, {
