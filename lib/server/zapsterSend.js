...
import { checkProactiveWhatsappAllowed, PROACTIVE_SKIP_REASON } from './proactiveWhatsappGate.js';
...
/**
 * @param {{ recipient: string; text: string; instanceId: string; proactive?: boolean; academyId?: string; leadId?: string; leadDoc?: object }} p
 */
export async function sendZapsterText({
  recipient,
  text,
  instanceId,
  proactive = false,
  ...
  if (proactive) {
    ...
    if (!gate.allowed) {
      return {
        ...
        erro: gate.message || PROACTIVE_SKIP_REASON,
        gate,
      };
    }
  }
