...
import { aggregateExperimentalByProfessor } from '../experimentalProfessor.js';
...
/**
 * Métricas de alunos apenas — uma passagem, sem listas/gráficos do funil.
 * @param {object[]} allLeads
 * @param {{ from: string, to: string, prevFrom: string, prevTo: string }} opts
 */
export function aggregateStudentMetricsOnly(allLeads, { from, to, prevFrom, prevTo }) {
  let activeAtStart = 0;
  let activeAtEnd = 0;
  let activeAtStartPrev = 0;
  let activeAtEndPrev = 0;
  let newStudents = 0;
  let newStudentsPrev = 0;
  let deactivations = 0;
  let deactivationsPrev = 0;

  const fromStart = new Date(from);
  const toEnd = new Date(to);
  const prevFromStart = new Date(prevFrom);
  ...
  for (const l of allLeads) {
    ...
    if (isActiveStudentAtDate(l, toEnd)) activeAtEnd += 1;
    if (isActiveStudentAtDate(l, prevFromStart)) activeAtStartPrev += 1;
    if (isActiveStudentAtDate(l, prevToEnd)) activeAtEndPrev += 1;
    if (newStudentIn) newStudents += 1;
    if (newStudentPrev) newStudentsPrev += 1;
    if (countsAsDeactivationInPeriod(l, from, to)) deactivations += 1;
    if (countsAsDeactivationInPeriod(l, prevFrom, prevTo)) deactivationsPrev += 1;
  }
  ...
  const prevChurn =
    activeAtStartPrev > 0 ? Math.round((deactivationsPrev / activeAtStartPrev) * 1000) / 10 : 0;
  ...
  return {
    studentMetrics: {
      ...
      retentionRate,
      previous: {
        activeAtStart: activeAtStartPrev,
        activeAtEnd: activeAtEndPrev,
        newStudents: newStudentsPrev,
        deactivations: deactivationsPrev,
        churnRate: prevChurn,
        retentionRate: Math.max(0, Math.round((100 - prevChurn) * 10) / 10),
      },
    },
...
export function aggregateLeadsReport(allLeads, { from, to, prevFrom, prevTo, chartMode = 'weekly' }) {
  ...
  for (const l of allLeads) {
    ...
    if (newStudentIn) {
      ...
      bumpBucketYmd(chartData, enrollmentDateYmd(l), 'converted');
    }
    ...
    if (newStudentPrev) bumpBucketYmd(prevChartData, enrollmentDateYmd(l), 'converted');
  }
  ...
  const experimentalByProfessor = aggregateExperimentalByProfessor(allLeads, { from, to });
