...
export async function revertSaleItemsStock({
  ...
  for (const it of itens) {
    ...
    const resolved = await resolveStockDocument(databases, DB_ID, STOCK_ITEMS_COL, lookupId);
    // resolveStockDocument returns `collection` (not collectionId). Use canonical $id —
    // lookup may be a legacy_stock_item_id that maps to a different variant doc.
