...
async function handleImport(req, res, academyId, me) {
  ...
  try {
    ...
    const msg = String(e?.message || '');
    if (!msg.includes('Unknown attribute') || !bankAccount) throw e;
    const { bank_account: _drop, ...lean } = statementPayload;
    statement = await databases.createDocument(
      DB_ID,
      BANK_STATEMENTS_COL,
      ID.unique(),
  ...
  const matchResults = matchBankItemsToTransactions(itemsWithBank, naviTx);
  ...
  const suggestedCount = matchResults.filter((r) => r.suggested_tx_id).length;
  const st = 'pending';
...
async function handleConfirmAll(req, res, academyId, me) {
  ...
  for (const item of itemsRes.documents || []) {
    ...
    if (!txId || item.status === 'ignored') continue;
    const txCheck = await fetchAndValidateTxForReconciliation(
      databases,
      DB_ID,
      FINANCIAL_TX_COL,
