...
import { mergePagbankIntoSettings } from '../pagbankSettings.js';
...
export async function fetchPagbankMaxRetries(pagbankToken, apiUrl = PAGBANK_API_URL) {
  try {
    const response = await fetch(`${apiUrl}/preferences/retries`, {
      headers: { Authorization: `Bearer ${pagbankToken}` },
    });
    if (!response.ok) {
      return parsePagbankMaxRetries(null);
    }
    const data = await parseFetchJson(response);
    return parsePagbankMaxRetries(data);
  } catch (e) {
    console.error(
      '[pagbankSetupHandler] failed to fetch retry preferences, using default',
      e?.message || e
    );
    return parsePagbankMaxRetries(null);
  }
...
export default async function pagbankSetupHandler(req, res) {
  ...
  try {
    ...
    const maxRetries = await fetchPagbankMaxRetries(validated.pagbank_token);
    ...
    const settingsPayload = mergePagbankIntoSettings(academyDoc.settings, {
      token_encrypted: encryptPagbankToken(validated.pagbank_token),
    ...
    await databases.updateDocument(DB_ID, ACADEMIES_COL, academyDoc.$id, {
      ...
      pagbank_max_retries: maxRetries,
    });
