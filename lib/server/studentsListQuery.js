...
} from '../../src/lib/phoneSearchQuery.js';
...
export const STUDENT_LIST_SELECT = [
  ...
  'converted_at',
  'enrollmentDate',
  // Apenas snake_case do schema — `dueDay` camelCase não existe e derruba Query.select
  // (fallback sem select em toda página = listagem bem mais lenta).
  'due_day',
  'discount_amount',
  'discount_type',
...
export function parseStudentsListQueryParams(query = {}) {
  ...
  const offset =
    Number.isFinite(offsetRaw) && offsetRaw > 0 ? Math.min(Math.trunc(offsetRaw), 5000) : undefined;
  const turmaEmpty =
    query.turma_empty === '1' ||
