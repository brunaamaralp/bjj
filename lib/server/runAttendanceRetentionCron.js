...
import { passesAudienceFilter } from '../../src/lib/automationAudience.js';
...
function shouldTriggerAutomation(metrics, automationKey, cfg) {
  ...
  if (automationKey === 'newcomer_at_risk') {
    return isNewcomer && metrics?.status === ATTENDANCE_RISK_STATUS.AT_RISK && days >= threshold;
  }
  if (automationKey === 'absent_student') {
    return metrics?.status === ATTENDANCE_RISK_STATUS.ABSENT || days >= threshold;
  }
  return false;
}
...
export async function runAttendanceRetentionCron(databases, dbId) {
  ...
  for (const academy of academies) {
    ...
    let classes = [];
    try {
      classes = await listAcademyClassDocs(databases, academy.$id);
    } catch {
      classes = [];
    }
    const goalsContext = buildWeeklyGoalsContext(financeConfig, classes);
    ...
    for (const raw of docs) {
      ...
      const metrics = buildStudentRetentionMetrics(student, lastCheckin, today, {
        checkinsLast7Days: checkinsLast7DaysByStudent.get(studentId) ?? 0,
        goalsContext,
      });
      if (!metrics) continue;
      ...
      const audience = cfg.audience ?? null;
      const studentForAudience = {
        ...student,
        id: studentId,
        academyId: academy.$id,
        type: raw.type ?? student.type,
        plan: raw.plan ?? student.plan,
        turma: raw.turma ?? student.turma,
        enrollmentDate: raw.enrollmentDate ?? student.enrollmentDate,
        converted_at: raw.converted_at ?? student.convertedAt,
      };
      if (
        !passesAudienceFilter(studentForAudience, audience, {
  ...
  console.log(`[cron] attendance-retention concluído em ${elapsedMs}ms`);

  return { sent, skipped, errors, details, elapsedMs };
}
