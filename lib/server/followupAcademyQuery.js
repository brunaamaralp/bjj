...
export async function enrichLeadsForFollowupQuery(databases, academyId, leads, academySettings) {
  ...
  const ctx = buildFollowupQueryContext(leads, bundle, academySettings);
  return enrichFollowUpLeads(
    (leads || []).filter((l) => excludeImported(l) && isFollowUpLead(l)),
    ctx
  );
}
...
export function queryAttendedNotContactedYesterdayRows(enriched, now = new Date()) {
  ...
  return enriched
    ...
    .map((l) => ({
      ...
      line: `${l.name} · experimental ${formatDateBr(l.scheduledDate)} · sem retorno em D+1`,
    }));
}
