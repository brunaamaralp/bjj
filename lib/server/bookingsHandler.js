 ...
 * GET  ?route=bookings&action=get-lesson-register
 * POST ?route=bookings&action=upsert-lesson-register
...
import {
  getOrCreateClassSlot,
  mapSlotLessonFields,
  upsertLessonRegister,
} from './lessonRegisterOps.js';
...
function lessonOpsErrorStatus(code) {
  if (code === 'bad_request') return 400;
  if (code === 'forbidden') return 403;
  if (code === 'not_found') return 404;
  if (code === 'misconfigured') return 503;
  return 500;
}

function lessonOpsErrorMessage(err) {
  const code = err?.code || err?.message;
  const map = {
    schedule_id_and_date_required: 'schedule_id e date são obrigatórios',
    access_denied: 'Acesso negado',
    schedules_not_configured: 'Horários não configurados',
    schedule_not_found: 'Horário não encontrado',
    schedule_not_on_date: 'Este horário não ocorre na data selecionada',
    instructor_required: 'Selecione o professor responsável',
  };
  return map[code] || apiErro(err, 'action');
}

async function handleGetLessonRegister(req, res, academyId) {
  const scheduleId = String(req.query?.schedule_id || '').trim();
  const slotDate = String(req.query?.date || req.query?.slot_date || '').trim();
  if (!scheduleId || !slotDate) {
    return json(res, 400, { sucesso: false, erro: 'schedule_id e date são obrigatórios' });
  }
  try {
    const { slot, created } = await getOrCreateClassSlot(
      databases,
      DB_ID,
      CLASS_SLOTS_COL,
      academyId,
    ...
    return json(res, 200, {
      sucesso: true,
      slot: mapSlotDoc(slot),
      created,
    });
  } catch (e) {
    return json(res, lessonOpsErrorStatus(e?.code), {
      sucesso: false,
      erro: lessonOpsErrorMessage(e),
    });
  }
}

async function handleUpsertLessonRegister(req, res, academyId, me) {
  try {
    const { slot } = await upsertLessonRegister(
      databases,
      DB_ID,
      CLASS_SLOTS_COL,
      academyId,
    ...
    return json(res, 200, { sucesso: true, slot: mapSlotDoc(slot) });
  } catch (e) {
    return json(res, lessonOpsErrorStatus(e?.code), {
      sucesso: false,
      erro: lessonOpsErrorMessage(e),
...
export default async function bookingsHandler(req, res) {
  ...
  const lessonOnly =
    action === 'get-lesson-register' || action === 'upsert-lesson-register';
  if (!lessonOnly && !BOOKINGS_COL) {
    return json(res, 503, { sucesso: false, erro: 'Agendamento não configurado' });
  }
  ...
  if (action === 'get-lesson-register' && req.method === 'GET') {
    return handleGetLessonRegister(req, res, academyId);
  }
  if (action === 'upsert-lesson-register' && req.method === 'POST') {
    return handleUpsertLessonRegister(req, res, academyId, me);
  }
