...
import { evaluateBillingAccess } from '../billing/gate.js';
...
export async function assertBillingActive(academyId) {
  ...
  if (!row) {
    throw new BillingGateError(
      402,
      'NO_SUBSCRIPTION',
      'Assinatura não encontrada. Acesse /conta?tab=assinatura para ativar.'
    );
  }
  ...
  throw new BillingGateError(
    ...
    'Assinatura vencida ou inativa. Acesse /conta?tab=assinatura para renovar.'
  );
}
