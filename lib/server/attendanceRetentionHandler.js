...
function collectStudentFilterOptions(students) {
  ...
  return {
    turmas: [...turmas].sort(sortPt),
    belts: [...belts].sort(sortPt),
  };
}
...
const RETENTION_STUDENT_SELECT_LEGACY = RETENTION_STUDENT_SELECT.filter(
  (key) => key !== 'retention_snoozed_until' && key !== 'retention_in_contact'
);

const STUDENT_QUERY_SELECT_ATTEMPTS = [RETENTION_STUDENT_SELECT, RETENTION_STUDENT_SELECT_LEGACY, null];
...
function buildActiveStudentQueries(academyId, cursor, selectAttrs) {
  const queries = [
  ...
  if (selectAttrs?.length) queries.splice(2, 0, Query.select(selectAttrs));
 ...
 * @param {string[]|null} selectAttrs
 */
async function fetchAllActiveStudentsWithSelect(db, academyId, selectAttrs) {
  const all = [];
  let cursor = null;
  ...
  for (;;) {
    const page = await db.listDocuments(
      DB_ID,
      STUDENTS_COL,
      buildActiveStudentQueries(academyId, cursor, selectAttrs)
    );
...
async function fetchAllActiveStudents(db, academyId) {
  ...
  let lastErr;
  for (const selectAttrs of STUDENT_QUERY_SELECT_ATTEMPTS) {
    try {
      return await fetchAllActiveStudentsWithSelect(db, academyId, selectAttrs);
    } catch (e) {
      lastErr = e;
      console.warn(
        '[attendance-retention] student query failed, retrying…',
        selectAttrs,
        e?.message || e
      );
    }
...
async function fetchAttendanceSince(db, academyId, sinceIso) {
  ...
  const runQuery = (withOrder) => {
    ...
    return (async () => {
      for (;;) {
        ...
        if (withOrder) queries.splice(2, 0, Query.orderDesc('checked_in_at'));
        if (cursor) queries.push(Query.cursorAfter(cursor));
  ...
  try {
    ...
    console.warn('[attendance-retention] ordered attendance query failed, retrying without sort', e?.message || e);
  ...
  try {
    ...
    if (ACADEMIES_COL) {
      try {
        ...
        console.warn('[attendance-retention] goals context unavailable', e?.message || e);
      }
    }
    ...
    const checkinsLast7DaysByStudent = aggregateCheckinsInWindowByStudent(attendanceDocs, undefined, today);
    const { summary, atRisk } = summarizeAttendanceRetention(filtered, lastCheckinByStudent, today, {
      checkinsLast7DaysByStudent,
      goalsContext,
    });
    const filterOptions = collectStudentFilterOptions(eligible);
    ...
    return json(res, 200, {
      ...
      filters: {
        turma: filters.turma || null,
        belt: filters.belt || null,
