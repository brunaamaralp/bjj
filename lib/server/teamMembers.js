import { Client, Users, Teams, ID, Databases, Account } from 'node-appwrite';
import { getAppwriteUserFromJwt } from './authAppwrite.js';
import { ensureAcademyAccess, isAcademyOwnerOrAdminUser, databases as sharedDb, DB_ID } from './academyAccess.js';
import {
  recordAcademyEvent,
  listAcademyEventsServer,
...
async function handleListEvents(req, res, sessionUser) {
  ...
  const { documents, total } = await listAcademyEventsServer(academyIdStr, { limit, offset });
...
async function handleCreateMember(req, res, sessionUser) {
  ...
  try {
    ...
    console.warn('[team] invite via email falhou, tentando criação com senha:', inviteErr?.message || inviteErr);
  ...
  return json(res, 200, {
    sucesso: true,
    inviteSent,
    tempPassword: inviteSent ? null : tempPassword,
...
async function handleUpdateMember(req, res, sessionUser) {
  ...
  if (newTeamRole) {
    if (!isOwner) {
      return json(res, 403, { erro: 'Apenas o titular pode alterar papéis' });
    }
