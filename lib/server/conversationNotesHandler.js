...
import { enrichRowsWithAuthorName, resolveUserDisplayName } from './resolveUserDisplayName.js';
...
function mapNote(d, authorName = '') {
  return {
    ...
    author_name: String(authorName || d.author_name || '').trim(),
...
export default async function handler(req, res) {
  ...
  if (method === 'GET') {
    ...
    try {
      ...
      const notes = await enrichRowsWithAuthorName(docs.map((d) => mapNote(d)));
  ...
  if (method === 'POST') {
    ...
    try {
      ...
      // PASSO 3: Auditoria + espelhar nota na timeline do lead (fire-and-forget)
      try {
        const convDoc = convOkPost.conv || {};
        ...
        recordAuditEvent({
          ...
          academyId,
          actor: actorFromMe({ $id: userId, name: authorName, email: me?.email }),
          target: { type: 'conversation', id: conversationId, name: String(convDoc.phone_number || '').trim() },
          context: {
            conversation_id: conversationId,
            ...(leadId ? { lead_id: leadId } : {}),
          },
          source: 'api.conversations.notes.post',
          payload: {
            note_id: created.$id,
            conversation_id: conversationId,
            body_preview: body.trim().slice(0, 200),
          },
          projectToLeadTimeline: leadId
            ? {
                leadId,
                type: 'inbox_note',
                text: body.trim(),
            ...
            : undefined,
        }).catch((e) => console.error('[conversationNotes] Erro ao registrar auditoria:', e?.message));
      } catch (e) {
        console.error('[conversationNotes] Falha ao preparar auditoria:', e?.message);
      }

      const authorName = String(me?.name || me?.email || 'Equipe').trim() || 'Equipe';
      return json(res, 201, { sucesso: true, note: mapNote(created, authorName) });
    } catch (e) {
  ...
  if (method === 'PATCH') {
    ...
    try {
      ...
      const authorName = await resolveUserDisplayName(updated.author_id);
      return json(res, 200, { sucesso: true, note: mapNote(updated, authorName) });
    } catch (e) {
      return json(res, 500, { sucesso: false, erro: apiErro(e, 'save') });
    }
  }
