...
export async function createPayableInstanceFromTemplate(databases, dbId, colId, template) {
  ...
  let doc;
  try {
    doc = await databases.createDocument(dbId, colId, ID.unique(), forDb, [
    ...
    doc = await databases.createDocument(dbId, colId, ID.unique(), stripUnknownFinanceTxAttrs(payload), [
      Permission.read(Role.users()),
      Permission.update(Role.users()),
    ]);
  }
