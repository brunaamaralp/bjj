...
import { ensureInitialPayableInstance } from './financeRecurrenceInstance.js';
    ...
    if (mapped?.is_recurrence_template && mapped.recurrence_type && mapped.recurrence_type !== 'none') {
      ...
      if (txDirection(mapped) === 'out' || isExpenseType(mapped.type)) {
        try {
          ...
          if (inst.created) invalidateFinanceForecastCache(academyId);
        } catch (e) {
...
export async function handlePatchFinanceTx(req, res, academyId, me, academyDoc, role) {
  ...
  if (action === 'settle') {
    ...
    const isOutflow =
      (prevMapped && txDirection(prevMapped) === 'out') || isExpenseType(prevType);
    if (isOutflow && !role.isAdmin) {
      return json(res, 403, {
        ok: false,
        error: 'Apenas titular ou administrador pode liquidar despesa',
      });
    }
    ...
    try {
      const doc = await databases.updateDocument(
        ...
        FINANCIAL_TX_COL,
        txId,
        financeTxDocumentForAppwrite(settlePatch)
      );
      const mapped = await mapAndEnrichTx(doc, academyId);
