...
export async function handleCreateFinanceTx(req, res, academyId, me, academyDoc, role) {
  ...
  try {
    ...
    try {
      ...
      const lean = stripUnknownFinanceTxAttrs(payload, msg);
...
export async function handlePatchFinanceTx(req, res, academyId, me, academyDoc, role) {
  ...
  if (action === 'update_recurrence') {
    ...
    const end = parseRecurrenceEnd(body.recurrence_end);
    patch.recurrence_end = end || '';
    try {
  ...
  try {
    ...
    const forDb = financeTxDocumentForAppwrite(patch);
    let doc;
    try {
      ...
      const msg = String(e?.message || '');
      if (!/unknown attribute/i.test(msg)) throw e;
      const lean = stripUnknownFinanceTxAttrs(patch, msg);
