...
- color: variação de cor (ex: Azul, Preto). Se não houver, use null.
...
function looksLikeNativeNaveHeaders(headers = []) {
  const set = new Set(headers.map(normalizeHeader).filter(Boolean));
  // padrão nativo do Nave (mínimo para parse direto sem IA)
  return set.has('nome') && set.has('categoria') && set.has('tamanho') && set.has('sale_price') && set.has('cost_price');
}

function nativeMappingFromHeaders(headers = []) {
  const byNorm = new Map();
  for (const h of headers) byNorm.set(normalizeHeader(h), String(h));
  const mapping = {};
  for (const f of PRODUCT_FIELDS) mapping[f] = null;
  mapping.nome = byNorm.get('nome') || null;
  mapping.categoria = byNorm.get('categoria') || null;
  mapping.Tamanho = byNorm.get('tamanho') || null;
  mapping.color = byNorm.get('color') || byNorm.get('cor') || null;
  mapping.descricao = byNorm.get('descricao') || byNorm.get('descrição') || null;
  mapping.sale_price = byNorm.get('sale_price') || null;
  mapping.cost_price = byNorm.get('cost_price') || null;
  mapping.initial_quantity = byNorm.get('initial_quantity') || null;
  mapping.minimum_level = byNorm.get('minimum_level') || null;
  mapping.unit = byNorm.get('unit') || null;
  mapping.sku = byNorm.get('sku') || null;
  mapping.is_for_sale = byNorm.get('is_for_sale') || null;
  mapping.unmapped = headers.filter((h) => !Object.values(mapping).includes(h));
  return {
    mapping,
    confidence: Object.fromEntries(Object.keys(mapping).filter((k) => k !== 'unmapped').map((k) => [k, mapping[k] ? 'high' : 'low'])),
    suggestions: 'Cabeçalhos no padrão do Nave — mapeamento automático aplicado.',
...
export default async function aiProductImportHandler(req, res) {
  ...
  if (looksLikeNativeNaveHeaders(headers)) {
    return res.status(200).json({ ...nativeMappingFromHeaders(headers), catalog_mode: catalogMode });
  }
  ...
  try {
        ...
        messages: [
          {
            role: 'user',
            content: [
              `Esta planilha tem os seguintes cabeçalhos: ${JSON.stringify(headers)}.`,
              `Exemplo da primeira linha: ${JSON.stringify(sample_rows?.[0] || {})}.`,
              '',
              'Mapeie cada coluna para um dos campos: nome, categoria, Tamanho, color, descricao, sale_price, cost_price, initial_quantity, minimum_level, unit, sku, is_for_sale.',
              'Dicas de sinônimos: cor, color, variação de cor → color.',
              '',
    ...
    if (!response.ok) {
      ...
      return res.status(422).json({
        ...
        hint: 'Use o modelo do Nave (nome, categoria, Tamanho, sale_price, cost_price) ou revise os cabeçalhos.',
        catalog_mode: catalogMode,
      });
    }
    ...
    if (!parsed || typeof parsed !== 'object') {
      return res.status(422).json({
        error: 'Não consegui interpretar essa planilha.',
        hint: 'Resposta da IA inválida. Baixe o modelo CSV do Nave e tente novamente.',
        catalog_mode: catalogMode,
      });
    }
    ...
    if (err?.name === 'AbortError') {
      return res.status(422).json({
        error: 'Não consegui interpretar essa planilha.',
        hint: 'A análise demorou demais. Baixe o modelo CSV do Nave e tente novamente.',
        catalog_mode: catalogMode,
      });
    }
    ...
    return res.status(422).json({
      error: 'Não consegui interpretar essa planilha.',
      hint: 'Erro ao consultar a IA. Baixe o modelo CSV do Nave e tente novamente.',
      catalog_mode: catalogMode,
    });
  }
}
