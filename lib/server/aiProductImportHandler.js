...
export default async function aiProductImportHandler(req, res) {
  ...
  if (!ANTHROPIC_API_KEY) {
    return res.status(200).json(emptyAiResponse('IA não configurada. Mapeie as colunas manualmente.'));
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res.status(200).json(emptyAiResponse('Não foi possível obter sugestão da IA. Mapeie manualmente.'));
    }

    const rawText = data.content?.[0]?.text || '';
    const parsed = extractJsonObject(rawText);
    if (!parsed || typeof parsed !== 'object') {
      return res.status(200).json(emptyAiResponse('Resposta da IA inválida. Mapeie as colunas manualmente.'));
    }
    ...
    return res.status(200).json(emptyAiResponse('Erro ao consultar a IA. Mapeie as colunas manualmente.'));
  }
}
