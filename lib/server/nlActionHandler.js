...
{
  "action": "register_payment" | "register_sale" | "adjust_stock" | "inventory_query" | "academy_query" | "register_expense" | "register_checkin" | "update_student" | "settle_transaction" | "update_payment" | "add_note" | "mark_attended" | "mark_missed" | "register_whatsapp" | "mark_enrolled" | "mark_lost" | "schedule_experimental" | "move_pipeline_stage" | "create_lead",
  ...
  "data": {
    ...
    "query_type": "top_sellers | slow_movers | stock_level | margin — inventory_query; enrolled_in_month | unpaid_tuition | overdue_tuition | new_leads | attended_experimental | scheduled_experimental — academy_query",
    "period": "week | month — academy_query (new_leads, attended_experimental, scheduled_experimental; padrão week)",
    "product_name_filter": "nome do produto — inventory_query (margin, stock_level)",
...
- academy_query: use para PERGUNTAS sobre alunos, mensalidades e funil; query_type conforme intenção; reference_month YYYY-MM; para new_leads/attended_experimental/scheduled_experimental use period week (semana) ou month (mês); NÃO use register_payment para consultas
...
- missing: array com nomes dos campos obrigatórios ausentes`;
  ...
  try {
    ...
    if (
      ...
      ['register_payment', 'register_sale', 'adjust_stock', 'add_note', 'register_expense', 'register_checkin', 'update_student', 'settle_transaction', 'update_payment'].includes(
        parsed.action
      )
    ) {
      return res.status(200).json({
    ...
    if (parsed.action === 'inventory_query') {
      ...
      try {
        ...
        return res.status(200).json({
          action: null,
          error: e?.message || 'Não foi possível consultar o relatório de estoque.',
        });
      }
    }
    ...
    if (parsed.action === 'academy_query') {
      ...
      try {
        ...
        return res.status(200).json({
          action: null,
          error: e?.message || 'Não foi possível consultar os dados da academia.',
        });
      }
    }
