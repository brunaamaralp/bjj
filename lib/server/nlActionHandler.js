...
import { answerAcademyQuery, inferAcademyQueryType } from './nlAcademyQuery.js';
...
import { extractNlPhoneSearchDigits, isNlPhoneLookupQuery } from '../../src/lib/phoneSearchQuery.js';
import { buildNlPhoneLookupResponse, resolveNlPhonePeople } from './nlPhoneLookup.js';
...
function matchStructuredNlQuery(text) {
  const normalized = normalizeNlText(text);
  if (!normalized) return null;
  if (/(quantos?|qtos?|numero de)\s+alunos?\s+ativos?/.test(normalized)) {
    return { kind: 'active_students_count', queryType: 'active_students_count' };
  }
  const queryType = inferAcademyQueryType(text);
  if (
    ['unpaid_tuition', 'overdue_tuition', 'missed_experimental', 'attended_experimental', 'checkins_today'].includes(
      queryType
    )
  ) {
...
function inferNlQueryDomain(text, explicitContext = '') {
  const normalized = normalizeNlText(text);
  const ctx = String(explicitContext || '').trim().toLowerCase();
  if (ctx === 'perfil') return 'cross_domain';
  if (ctx === 'financeiro') return 'finance';
  if (ctx === 'funil') return 'students';
  if (ctx === 'vendas') return 'inventory';

  const financeHit = /(mensalidade|pagou|pagamento|inadimpl|financeir|caixa|receb|transac|despesa|resumo)/.test(normalized);
  const studentHit =
    /(aluno|alunos|matricul|check-?in|checkin|presenca|presen[çc]a|compareceu|faltou|experimental|lead|funil)/.test(
      normalized
    );
  const inventoryHit = /(estoque|produto|produtos|kimono|rashguard|venda|vendeu|margem|saldo do estoque)/.test(
    normalized
  );

  const hits = [financeHit, studentHit, inventoryHit].filter(Boolean).length;
  if (hits > 1) return 'cross_domain';
  if (inventoryHit) return 'inventory';
  if (financeHit) return 'finance';
  if (studentHit) return 'students';
  return 'cross_domain';
}
...
export default async function nlActionHandler(req, res) {
  ...
  const showFinance =
    queryDomain === 'finance' || queryDomain === 'cross_domain';
  const showFunnel = queryDomain === 'students' || queryDomain === 'cross_domain';
  const showStudents = queryDomain === 'students' || queryDomain === 'cross_domain';
  const showInventory = queryDomain === 'inventory' || queryDomain === 'cross_domain';
  const showLeads =
    showFunnel && /(funil|lead|experimental|compareceu|faltou|pipeline|etapa)/.test(normalizeNlText(text));
  ...
  const phoneSearchDigits = extractNlPhoneSearchDigits(text);
  if (phoneSearchDigits && isNlPhoneLookupQuery(text)) {
    try {
      ...
      const hits = await resolveNlPhonePeople(databases, academyId, phoneSearchDigits, clientPeople, {
        limit: 8,
      });
      const out = buildNlPhoneLookupResponse(hits, phoneSearchDigits);
      return res.status(200).json({
        action: 'academy_query',
        confidence: 'high',
        data: { ...out },
        summary: String(out.resposta || '').split('\n')[0].slice(0, 200),
        missing: [],
        warnings: [],
      });
    } catch (e) {
      console.error('[nl-action]', JSON.stringify({ stage: 'direct_phone_lookup', message: String(e?.message || e).slice(0, 200) }));
    }
  }
  ...
  if (directMatch?.kind === 'academy_query') {
    try {
      const out = await answerAcademyQuery(databases, {
        ...
        isOwnerOrAdmin: await isAcademyOwnerOrAdminUser(access.doc, me),
      });
      return res.status(200).json({
        action: 'academy_query',
        confidence: 'high',
      ...
      console.error('[nl-action]', JSON.stringify({ stage: 'direct_academy_query', message: String(e?.message || e).slice(0, 200) }));
    }
  }

  if (directMatch?.kind === 'active_students_count') {
    try {
      const academyStudents = await listAcademyStudentsMapped(academyId);
      const activeStudents = filterActiveStudents(academyStudents);
      return res.status(200).json({
        action: 'academy_query',
        confidence: 'high',
        data: {
          resposta: `${activeStudents.length} aluno(s) ativos no momento.`,
          rows: [],
          count: activeStudents.length,
          query_type: 'active_students_count',
          reference_month: currentMonth,
        },
        summary: `${activeStudents.length} aluno(s) ativos no momento.`,
        missing: [],
        warnings: [],
      });
    } catch (e) {
      console.error('[nl-action]', JSON.stringify({ stage: 'direct_active_students_count', message: String(e?.message || e).slice(0, 200) }));
    }
  }
  ...
  try {
    const enriched = await enrichNlActionContext(databases, {
      ...
      showFinance,
      showFunnel,
      clientPending: showFinance ? pendingTransactionsRaw : [],
      clientPayments: showFinance ? recentPaymentsRaw : [],
      clientStages: showFunnel ? pipelineStagesRaw : [],
      referenceMonth: currentMonth,
    });
  ...
  const stockProductsNorm = showInventory
  ...
  const studentsArr = Array.isArray(students) ? students : [];
  const studentList = showStudents ? buildStudentList(students) : '';
  const leadList = showLeads ? buildLeadList(leads) : '';

  const promptBlocks = [
    `Academia: ${String(academyName || '').trim()}`,
    `Data de hoje: ${today}`,
    `Mês atual: ${currentMonth}`,
    `Contexto solicitado: ${normalizedContext || (queryDomain === 'cross_domain' ? 'geral' : queryDomain)}${normalizedContext === 'perfil' ? ' (perfil: financeiro + funil habilitados no mesmo comando)' : ''}`,
  ];

  if (showStudents) {
    promptBlocks.push(`Alunos cadastrados:\n${studentList || 'Nenhum aluno cadastrado'}`);
  }
  if (showLeads) {
    promptBlocks.push(`Leads no funil:\n${leadList || 'Nenhum lead no funil'}`);
  }
  if (showFunnel) {
    promptBlocks.push(
      `Etapas do funil (para move_pipeline_stage use target_stage_id = id exato de uma linha abaixo):\n${pipelineStageLines || 'Nenhuma etapa enviada — não use move_pipeline_stage.'}`
    );
  }
  if (showFinance) {
    promptBlocks.push(
      pendingTxLines
        ? `Transações financeiras PENDENTES (para settle_transaction use transaction_id = id exato):\n${pendingTxLines}`
        : 'Transações pendentes: nenhuma no momento — não use settle_transaction.'
    );
    promptBlocks.push(
      paymentLinesForNl
        ? `Registros de mensalidade visíveis (para update_payment use payment_id = id exato):\n${paymentLinesForNl}`
        : 'Mensalidades do mês: nenhum registro — não use update_payment.'
    );
  }
  if (showInventory) {
    promptBlocks.push(
  ...
  if (showFinance && financePlanLines) {
    promptBlocks.push(`Planos da academia (valores de referência para mensalidades):\n${financePlanLines}`);
  }

  const coreActionLines =
    '1. register_payment — registrar pagamento de mensalidade de um aluno (ex.: "Deivid pagou a mensalidade de maio, pix")\n' +
...
- update_student: student_id obrigatório; campo updates (objeto) com pelo menos uma chave permitida: plan, enrollmentDate, birthDate (YYYY-MM-DD), cpf, responsavel, emergencyContact, emergencyPhone, preferredPaymentMethod, preferredPaymentAccount, name, phone, type (Adulto|Criança|Juniores), parentName, belt, origin
