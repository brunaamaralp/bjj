...
${showFinance ? '1. register_payment — registrar pagamento de mensalidade de um aluno (ex.: "Deivid pagou a mensalidade de maio, pix")\n2. register_sale — registrar venda de produto do estoque (ex.: "vendi rashguard branca M para o Deivid, pix")\n3. adjust_stock — ajustar saldo por perda/correção SEM venda (ex.: "quebramos 2 bonés", "sumiu uma camisa G", "ajusta estoque da camisa M para 5")\n4. add_note — adicionar uma nota sobre um lead ou aluno\n5. register_expense — registrar despesa/saída (compras, estoque, frutas, material, etc.)\n6. register_checkin — registrar presença (check-in) de um aluno\n7. update_student — atualizar dados do aluno (só campos permitidos em updates)\n8. settle_transaction — liquidar transação pendente (transaction_id da lista acima)\n9. update_payment — alterar observação (note), conta (account) ou nome do plano (plan_name) de um registro de mensalidade (payment_id da lista de mensalidades)' : ''}
...
{
  "action": "register_payment" | "register_sale" | "adjust_stock" | "register_expense" | "register_checkin" | "update_student" | "settle_transaction" | "update_payment" | "add_note" | "mark_attended" | "mark_missed" | "register_whatsapp" | "mark_enrolled" | "mark_lost" | "schedule_experimental" | "move_pipeline_stage" | "create_lead",
