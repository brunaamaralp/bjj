...
export function studentDocToReportPerson(doc) {
  ...
  return {
    ...
    experimental_professor_user_id: String(doc.experimental_professor_user_id || '').trim() || null,
    experimental_professor_name: String(doc.experimental_professor_name || '').trim() || null,
  };
}
...
export function mergeLeadsAndStudentsForReport(leadDocs, studentDocs) {
  ...
  for (const doc of studentDocs || []) {
    ...
    byId.set(id, {
      ...normalized,
      attended_at: pick(normalized.attended_at, prev?.attended_at),
      missed_at: pick(normalized.missed_at, prev?.missed_at),
      scheduledDate: pick(normalized.scheduledDate, prev?.scheduledDate),
      scheduledTime: pick(normalized.scheduledTime, prev?.scheduledTime),
      experimental_professor_user_id: pick(
        normalized.experimental_professor_user_id,
        prev?.experimental_professor_user_id
      ),
      experimental_professor_name: pick(
        normalized.experimental_professor_name,
        prev?.experimental_professor_name
      ),
    });
  }
  return [...byId.values()];
