...
export async function fetchAllReportPeople(databases, dbId, academyId, filters, signal) {
  ...
  const leadDocs = LEADS_COL
    ? await fetchAllCollectionDocuments(databases, dbId, LEADS_COL, leadQueries, signal)
    : [];

  let studentDocs = [];
  if (STUDENTS_COL) {
    const studentQueries = buildReportPeopleQueries({ collection: 'students', academyId, filters });
    studentDocs = await fetchAllCollectionDocuments(
      databases,
      dbId,
      STUDENTS_COL,
      studentQueries,
