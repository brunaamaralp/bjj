...
export async function registerInstanceWebhooks(instanceId, host, academyId = '') {
  ...
  if (
    ...
    detailLower.includes('instance_not_found')
  ) {
    const err = new Error('Inst\u00e2ncia n\u00e3o encontrada na Zapster');
    err.code = 'ZAPSTER_INSTANCE_NOT_FOUND';
    throw err;
  }
  ...
  if (
    resp.status === 409 ||
    (detailLower.includes('already') && detailLower.includes('webhook'))
  ) {
    console.log('[zapsterInstances] webhook já registrado para instância', { instanceId: id, webhookUrl });
    return { sucesso: true, alreadyRegistered: true };
  }
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST' && action === 'register-webhooks') {
    ...
    try {
      const live = await zapsterGetInstance(instanceIdBody);
      if (!live.ok) {
        if (live.status === 404) {
          return res.status(404).json({
            sucesso: false,
            erro: 'Inst\u00e2ncia n\u00e3o encontrada na Zapster',
            codigo: 'ZAPSTER_INSTANCE_NOT_FOUND',
          });
        }
        console.warn('[zapsterInstances] register-webhooks: Zapster retornou erro não-404 no pre-check', {
          instanceId: instanceIdBody,
          zapsterStatus: live.status,
        });
      }
      ...
      if (e?.code === 'ZAPSTER_INSTANCE_NOT_FOUND') {
        return res.status(404).json({
          sucesso: false,
          erro: e.message,
          codigo: e.code,
  ...
  if (req.method === 'GET') {
    if (action === 'qrcode') {
      ...
      try {
        ...
        const msg = isZapsterTimeoutError(e) ? ZAPSTER_TIMEOUT_FRIENDLY : apiErro(e, 'action');
    ...
    try {
      ...
      const msg = isZapsterTimeoutError(e) ? ZAPSTER_TIMEOUT_FRIENDLY : apiErro(e, 'load');
      return res.status(500).json({ sucesso: false, erro: msg, codigo: isZapsterTimeoutError(e) ? 'zapster_timeout' : undefined });
    }
  }
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      const msg = isZapsterTimeoutError(e) ? ZAPSTER_TIMEOUT_FRIENDLY : apiErro(e, 'save');
      return res.status(500).json({ sucesso: false, erro: msg, codigo: isZapsterTimeoutError(e) ? 'zapster_timeout' : undefined });
    }
  }
  ...
  if (req.method === 'DELETE') {
    ...
    try {
      ...
      return res.status(500).json({ sucesso: false, erro: apiErro(e, 'delete') });
    }
  }
