...
async function zapsterCreateInstance({ name, metadata, webhooks, host }) {
  ...
  if (resp.ok) {
    ...
    if (instanceId && String(host || '').trim()) {
      try {
        await registerInstanceWebhooks(instanceId, host);
...
/** Eventos enviados ao criar instância ou registrar webhook (alinhado à doc Zapster POST /instances + POST /instances/:id/webhooks). */
export const ZAPSTER_INSTANCE_WEBHOOK_EVENTS = ['message.received', 'instance.qrcode', 'instance.status'];
 ...
 */
export function buildWebhookAbsoluteUrl(host) {
 ...
 */
export async function registerInstanceWebhooks(instanceId, host) {
  const id = String(instanceId || '').trim();
  if (!id) throw new Error('instanceId obrigat\u00f3rio');
  const webhookUrl = buildWebhookAbsoluteUrl(host);
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST' && action === 'register-webhooks') {
    ...
    const bodyHost = String(parsedBody.host || '').trim();
    const hostForWebhook = bodyHost || fwdHost || hdrHost;
    if (!hostForWebhook) {
      return res.status(400).json({
        sucesso: false,
    ...
    try {
      await registerInstanceWebhooks(instanceIdBody, hostForWebhook);
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      const z = await zapsterCreateInstance({
        ...
        host: hostFromBody || String(req.headers.host || '').trim()
      });
      if (!z.ok) {
