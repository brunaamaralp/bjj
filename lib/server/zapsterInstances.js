...
async function persistInstanceId(academyId, instanceId, attempt = 0) {
  ...
  try {
    try {
      ...
      console.error('[zapsterInstances] primeiro updateDocument (zapster_instance_id + zapsterInstanceId) falhou:', e1);
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    if (action === 'qrcode') {
      ...
      const academyByInst = await getAcademyByInstanceId(idParam);
      if (!academyByInst) {
        return res.status(404).json({ sucesso: false, erro: 'Inst\u00e2ncia n\u00e3o encontrada' });
      }
