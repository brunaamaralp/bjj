...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    if (action === 'qrcode') {
      ...
      let academyByInst = await getAcademyByInstanceId(idParam);
      // Fallback: o id j\u00e1 est\u00e1 no documento da academia do header (evita 404 se listDocuments falhar \u2014 ex.: \u00edndice Appwrite ausente).
      if (!academyByInst && academyLinkedInstanceId(doc) === idParam) {
        academyByInst = doc;
      }
      if (!academyByInst) {
