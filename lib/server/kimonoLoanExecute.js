...
function isKimonoLoansSchemaError(e) {
  const msg = String(e?.message || e || '');
  return (
    /collection.*not found|could not be found/i.test(msg) ||
    /attribute.*not found|unknown attribute/i.test(msg) ||
    /index.*not found|missing index/i.test(msg)
  );
}
...
async function listActiveKimonoLoans(databases, dbId, academyId) {
  ...
  try {
    ...
    docs.sort((a, b) => {
      const ta = Date.parse(String(a.lent_at || a.$createdAt || '')) || 0;
      const tb = Date.parse(String(b.lent_at || b.$createdAt || '')) || 0;
      return tb - ta;
    });
    return docs;
  } catch (e) {
    if (isKimonoLoansSchemaError(e)) {
