...
const INVALID_VALUE_ATTR_RES = [
  /Invalid document structure:\s*Attribute\s+"([^"]+)"\s+has invalid value/i,
  /Invalid document structure:\s*Attribute\s+'([^']+)'\s+has invalid value/i,
];
...
/** Só atributos ausentes no schema — não remover campos com valor/tipo inválido. */
...
export function parseInvalidAttributeFromMessage(msg) {
  const text = String(msg || '');
  for (const re of INVALID_TYPE_ATTR_RES) {
    const m = text.match(re);
    if (m) return String(m[1] || '').trim();
  }
  for (const re of INVALID_VALUE_ATTR_RES) {
    const m = text.match(re);
    if (m) return String(m[1] || '').trim();
  }
  return null;
