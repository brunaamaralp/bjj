...
- low_confidence: true se a leitura da linha for incerta (data, valor ou direção ambíguos)
...
{
  "items": [
    { "date": "2026-01-15", "description": "PIX João Silva", "amount": 150.00, "direction": "credit", "low_confidence": false }
  ],
...
async function callAnthropic({ system, userContent, pdfBase64 = null, academy_id = '' }) {
  ...
  try {
    ...
    if (mode === 'pdf') {
      ...
      rawText = await callAnthropic({
        ...
        pdfBase64: b64,
        academy_id: String(access.academyId || '').trim(),
      });
    } else {
