...
import { roundMoney, normalizePaymentForma } from './salePayments.js';
...
export async function assertCashShiftForSale(academyDoc, academyId, { saleSource = 'pdv' } = {}) {
  ...
  const src = normalizeSaleSource(saleSource);
  const requiredFor = settings.cashShiftRequiredFor ?? ['pdv'];
  if (!requiredFor.includes(src)) return { ok: true, shiftId: null };
  const open = await findOpenCashShift(academyId);
