...
export async function loadClosingGetPayload(academyId, referenceMonth, regime) {
  const [payments, txResult, cashClosing] = await Promise.all([
    listPaymentsForMonth(academyId, referenceMonth),
    listFinancialTxForMonth(academyId, referenceMonth, regime),
    getCashClosing(academyId, referenceMonth),
  ]);
  return {
    referenceMonth,
    regime,
