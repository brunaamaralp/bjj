import { AGENT_SYSTEM_PROMPT_MAX_CHARS } from '../aiPromptLimits.js';
...
CAMPO "resposta" NO JSON — SEMPRE TEXTO LIVRE (CONVERSA):
- O valor de "resposta" é o que vai para o WhatsApp, copiado como mensagem de chat
- Só frases corridas, como se estivesse digitando na conversa — nunca formato de documento
- Proibido dentro de "resposta": markdown (**, *, #, __), listas com traço ou número,
  tabelas, blocos de código, títulos ou rótulos ("Resumo:", "Pontos:", "Obs:")
- Proibido colocar JSON, HTML ou explicações sobre classificação dentro de "resposta"
- Se precisar passar mais de um detalhe, una em frases ("A experimental é grátis e dura uns 40 min.")
  em vez de lista numerada ou com marcadores
...
export function buildClassificationBlock() {
  return [
    ...
    'O campo "resposta" deve ser APENAS texto livre de conversa (uma ou poucas mensagens curtas de WhatsApp), sem markdown, sem listas estruturadas e sem metadados — a classificação fica só em "classificacao".',
    'Retorne SOMENTE um JSON válido (sem markdown ao redor do JSON) no seguinte formato:',
...
/**
 * Monta o system prompt e trunca FAQ (depois corpo) se ultrapassar o teto da janela.
 * @returns {{ system: string; truncated: boolean; warning?: string }}
 */
export function assembleAgentSystemPromptBounded(params, maxChars = AGENT_SYSTEM_PROMPT_MAX_CHARS) {
  let faqItems = Array.isArray(params.faqItems) ? [...params.faqItems] : [];
  let truncated = false;
  let system = assembleAgentSystemPrompt({ ...params, faqItems });

  const cap = Math.max(8000, Number(maxChars) || AGENT_SYSTEM_PROMPT_MAX_CHARS);
  if (system.length <= cap) return { system, truncated: false };

  while (faqItems.length > 0 && system.length > cap) {
    faqItems = faqItems.slice(0, Math.max(0, faqItems.length - 1));
    system = assembleAgentSystemPrompt({ ...params, faqItems });
    truncated = true;
  }

  if (system.length > cap) {
    const body = String(params.effectiveBody || '');
    const shorterBody = body.slice(0, Math.max(0, body.length - (system.length - cap) - 64));
    system = assembleAgentSystemPrompt({ ...params, faqItems, effectiveBody: shorterBody });
    truncated = true;
  }
  ...
  return {
    ...
    truncated,
    warning: truncated
      ? 'O prompt montado foi reduzido automaticamente (FAQ ou trechos longos) para caber no limite do modelo.'
      : undefined,
  };
}
