...
import { handleKimonoLoansGet, handleKimonoLoansPost } from './kimonoLoanHandler.js';
...
export default async function inventoryHandler(req, res) {
  ...
  const kimonoLoansGet = method === 'GET' && String(req.query.kimono_loans || '').trim() === '1';
  const kimonoLoansPost =
    method === 'POST' &&
    req.body &&
  ...
  if (kimonoLoansGet) {
    return handleKimonoLoansGet(req, res, kimonoCtx);
  }
  if (kimonoLoansPost) {
    return handleKimonoLoansPost(req, res, kimonoCtx);
  }
