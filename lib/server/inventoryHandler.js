...
function mapStockMoveDoc(doc) {
  return {
    id: doc.$id,
    tipo: String(doc.tipo || '').toLowerCase(),
    quantidade: Number(doc.quantidade) || 0,
    motivo: String(doc.motivo || '').trim(),
    usuario_id: String(doc.usuario_id || '').trim(),
    referencia_id: String(doc.referencia_id || '').trim(),
...
export async function handleListStockMovesForItem(res, academyId, itemId) {
  ...
  try {
    ...
    const moves = (list.documents || []).map(mapStockMoveDoc);
    return json(res, 200, { sucesso: true, moves });
  } catch (e) {
    console.error('[inventory] moves:', itemId, e?.message || e);
    return json(res, 500, { sucesso: false, erro: e?.message || 'Erro ao listar movimentações' });
  }
}
        ...
        const item =
          resolved.collection === PRODUCT_VARIANTS_COL
            ? {
                ...
                status: computeStockStatus(resolveCurrentQuantity(updated), updated.minimum_level),
