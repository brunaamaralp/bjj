...
/** FAQ fictícia no sandbox — não expõe dados operacionais da academia. */
const SANDBOX_FAQ_ITEMS = [
  { q: 'Qual o horário de funcionamento?', a: 'Segunda a sexta das 7h às 22h (exemplo de teste).' },
  { q: 'Tem aula experimental?', a: 'Sim, a experimental é gratuita e dura cerca de 40 minutos (exemplo).' },
  { q: 'Onde fica a academia?', a: 'Endereço fictício para teste — configure o endereço real no prompt.' },
];
...
async function callClaude({ system, messages, maxTokens, temperature }, attempt = 0) {
  ...
  const timeoutId = setTimeout(() => controller.abort(), CLAUDE_TEST_TIMEOUT_MS);
  try {
    ...
    if (name === 'AbortError') {
      const err = new Error(`Timeout na chamada ao Claude após ${CLAUDE_TEST_TIMEOUT_MS}ms`);
      err.code = 'timeout';
      throw err;
    }
    throw e;
  ...
  try {
    ...
    if (!effectiveIntro && !effectiveBody) {
      return res.status(400).json({
        sucesso: false,
        erro: 'prompt_nao_configurado',
        code: 'prompt_nao_configurado',
      });
    }

    const faqItems = SANDBOX_FAQ_ITEMS;
    ...
    try {
      ...
      const code = e?.code === 'timeout' ? 'timeout' : 'upstream_error';
      const status = code === 'timeout' ? 504 : 502;
      return res.status(status).json({
        sucesso: false,
        erro: apiErro(e, 'action'),
        code,
      });
    }
    ...
    try {
      ...
      logApiError('agentTest counter', e);
    ...
    console.error('[agentTest] erro:', error?.message, error?.status);
    if (!res.headersSent) {
      return res.status(500).json({
        sucesso: false,
        erro: apiErro(error, 'action'),
      });
    }
  }
}
