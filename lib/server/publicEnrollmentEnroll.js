...
import { snapshotPlanPriceFromCatalog } from '../../src/lib/planBilling.js';
import { parseFinanceConfigRaw } from '../../src/lib/financeConfigStorage.js';
...
/** @param {{ type?: string, birthDate?: string }} form */
export function validateEnrollmentBirthDate(form) {
  const profileType = String(form?.type || 'Adulto').trim() || 'Adulto';
  const birthDate = String(form?.birthDate || '').trim().slice(0, 10);
  if (profileType === 'Criança' || profileType === 'Juniores') {
  ...
  if (birthDate && !BIRTH_DATE_YMD.test(birthDate)) {
...
function buildFormOverrides(form, customAnswersJson, academyDoc) {
  ...
  const overrides = {
    ...
    isFirstExperience: String(form.isFirstExperience || 'Sim').trim() || 'Sim',
  ...
  const belt = resolvePublicEnrollmentBelt(form, academyDoc?.settings);
  if (belt) overrides.belt = belt;
...
export async function enrollPublicStudent(databases, academyDoc, academyId, form, questions = []) {
  ...
  const birthCheck = validateEnrollmentBirthDate(form);
  if (!birthCheck.ok) {
    const err = new Error(birthCheck.code);
    err.code = birthCheck.code;
    err.message = birthCheck.message;
    throw err;
  }
  ...
  const overrides = buildFormOverrides(form, customAnswersJson, academyDoc);
