...
function buildDocumentPermissions(academyDoc) {
...
async function findDocByPhone(databases, collectionId, academyId, phone) {
  ...
  for (const variant of phoneQueryVariants(phone)) {
    try {
      ...
      for (const doc of res.documents || []) {
        if (normalizeEnrollmentPhone(doc.phone) === normalizeEnrollmentPhone(phone)) return doc;
      }
    } catch {
      void 0;
    }
...
export async function enrollPublicStudent(databases, academyDoc, academyId, form, questions = []) {
  ...
  const perms = buildDocumentPermissions(academyDoc);
  const notes = String(form.notes || '').trim().slice(0, 2000);

  const existingStudent = await findDocByPhone(databases, STUDENTS_COL, academyId, phone);
  if (existingStudent) {
  ...
  const existingLead = LEADS_COL ? await findDocByPhone(databases, LEADS_COL, academyId, phone) : null;
