...
import { mirrorSaleFinancials, mirrorSaleFinancialsForDoc } from './salesMirror.js';
...
async function loadStockItem(itemId, academyId) {
  ...
  return { doc, col: resolved.collection || STOCK_ITEMS_COL, parent: resolved.parent };
}
...
export default async function salesCreateHandler(req, res) {
  ...
  try {
    for (const it of itens) {
      ...
      const salePrice = Number(loaded.parent?.sale_price ?? stock.sale_price ?? stock.preco_venda);
      const costPrice = Number(loaded.parent?.cost_price ?? stock.cost_price ?? stock.preco_custo);
      ...
      if (salesSettings.lockPriceEdit && !collaborator) {
        ...
        const check = validateCollaboratorPrice(unit, stock, isOwner);
  ...
  const paymentStatusAtMove = derivePaymentStatusAtMove(pagamentosNorm, totalRounded);
  ...
  try {
    for (const it of normalizedLines) {
      ...
      stockSnapshots[it.item_estoque_id] = { current_quantity: prevQty, col: stockCol };
      ...
      const parentName = loaded.parent?.nome || itemDisplayName(itemDoc);
      const vLabel =
      ...
      if (STOCK_MOVES_COL) {
        ...
        const movePayload = buildSaleStockMovePayload({
          ...
          cmvUnit: cmvUnitFromTotals(cmvResult.cmv, it.quantidade, itemDoc),
        });
    ...
    for (const [itemId, snap] of Object.entries(stockSnapshots)) {
      try {
        await databases.updateDocument(DB_ID, STOCK_ITEMS_COL, itemId, {
