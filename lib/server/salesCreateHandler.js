...
async function salesCreateHandlerCore(req, res) {
  ...
  if (deferred) {
    ...
    const check = validatePagamentosForSettlement(pagamentosNorm, totalRounded, { allowPartial: true });
    if (!check.ok) {
      return json(res, 400, {
        ok: false,
  ...
  try {
    ...
    const finalStatus = deferred ? 'pendente' : check.isComplete ? 'concluida' : 'parcial';
