...
async function salesCreateHandlerCore(req, res) {
  ...
  const reservedQty = Object.create(null);
  ...
  try {
    for (const it of itens) {
      ...
      const disponivel = availableAfterReservation(
        stock,
        lineKind,
        parentType,
        reservedQty,
      ...
      if (Number.isFinite(expected) && expected !== availableQuantityForLineKind(stock, lineKind, parentType)) {
        return json(res, 409, {
          error: 'stock_stale',
          item_estoque_id: it.item_estoque_id,
          disponivel: availableQuantityForLineKind(stock, lineKind, parentType),
          expected,
          line_kind: lineKind,
          nome: itemDisplayName(stock),
        });
      ...
      reserveLineQuantity(reservedQty, canonicalId, lineKind, q);
  ...
  try {
    for (const it of normalizedLines) {
      ...
      let itemDoc = loaded.doc;
      const stockCol = loaded.col || STOCK_ITEMS_COL;
      const parentType = loaded.parent?.type || itemDoc.type || 'sale';

      // Re-ler antes da baixa para reduzir janela de corrida entre vendas.
      try {
        const fresh = await loadStockItem(it.item_estoque_id, academyId);
        itemDoc = fresh.doc;
        loaded.doc = itemDoc;
        loaded.col = fresh.col || loaded.col;
      } catch {
        /* usa snapshot já validado */
      }

      const disponivelNow = availableQuantityForLineKind(itemDoc, it.line_kind, parentType);
      if (disponivelNow < it.quantidade) {
        throw Object.assign(new Error('no_stock_race'), {
          code: 'NO_STOCK_RACE',
          item_estoque_id: it.item_estoque_id,
          disponivel: disponivelNow,
          line_kind: it.line_kind,
          nome: itemDisplayName(itemDoc),
        });
      }
      ...
      // Snapshot só na 1ª linha do SKU — rollback deve restaurar o estado pré-venda.
      if (!stockSnapshots[it.item_estoque_id]) {
      ...
      // Mantém doc em memória para linhas seguintes do mesmo SKU (sale+rental / qty split).
      loaded.doc = applySaleStockPatchInMemory(itemDoc, it.quantidade, it.line_kind);
    ...
    if (err?.code === 'NO_STOCK_RACE') {
      return json(res, 409, {
        ok: false,
        error: 'no_stock',
        item_estoque_id: err.item_estoque_id,
        disponivel: err.disponivel,
        line_kind: err.line_kind,
        nome: err.nome,
      });
    }
