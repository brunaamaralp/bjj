...
export default async function salesCreateHandler(req, res) {
  ...
  try {
    for (const it of itens) {
      const stock = await loadStockItem(String(it.item_estoque_id), academyId);
      stockDocs[it.item_estoque_id] = stock;
      ...
      const salePrice = Number(stock.sale_price ?? stock.preco_venda);
      const costPrice = Number(stock.cost_price ?? stock.preco_custo);
  ...
  try {
    for (const it of normalizedLines) {
      await databases.createDocument(DB_ID, SALE_ITEMS_COL, ID.unique(), {
      ...
      const itemDoc = stockDocs[it.item_estoque_id];
      const prevQty = resolveCurrentQuantity(itemDoc);
      stockSnapshots[it.item_estoque_id] = { current_quantity: prevQty };
      ...
      if (STOCK_MOVES_COL) {
        await databases.createDocument(DB_ID, STOCK_MOVES_COL, ID.unique(), {
          item_estoque_id: it.item_estoque_id,
          tipo: 'saida_venda',
          quantidade: it.quantidade,
