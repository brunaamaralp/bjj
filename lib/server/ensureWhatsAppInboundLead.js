...
import { isConversationTriageDismissed } from './conversationTriageDismissed.js';
...
async function getConversationForTriageCheck(databases, academyId, phone, conversationDocId = '') {
  if (!databases || !CONVERSATIONS_COL || !DB_ID) return null;
  const a = String(academyId || '').trim();
  if (!a) return null;

  const cid = String(conversationDocId || '').trim();
  if (cid) {
    try {
      const doc = await databases.getDocument(DB_ID, CONVERSATIONS_COL, cid);
      if (doc && String(doc.academy_id || '').trim() === a) return doc;
    } catch {
      void 0;
    }
  }
...
export async function ensureWhatsAppInboundLead({
  ...
  const conversationDoc = await getConversationForTriageCheck(
    databases,
    a,
    phone,
    conversationDocId
  );
  if (isConversationTriageDismissed(conversationDoc)) {
    return { leadDoc: null, created: false, skippedReason: 'triage_dismissed' };
  }
