  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    try {
      ...
      try {
        ...
        if (userAdds > 0) {
          minimal.last_user_msg_at = payload.last_user_msg_at || nowIso;
          minimal.unread_count = payload.unread_count;
        }
...
/**
 * Atualiza mídia de mensagem assistant existente (ex.: placeholder de envio pelo celular).
 * @param {string} docId
 * @param {string} messageId
 * @param {Record<string, unknown>} patch
 * @param {{ onlyIfPlaceholder?: boolean }} [opts]
 */
export async function patchAssistantMessageMedia(docId, messageId, patch, opts = {}) {
  const id = String(docId || '').trim();
  const mid = String(messageId || '').trim();
  if (!databases || !DB_ID || !CONVERSATIONS_COL || !id || !mid) {
    return { ok: false, erro: 'config_ou_id_invalido' };
  }
  if (!patch || typeof patch !== 'object') return { ok: false, erro: 'patch_invalido' };
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    try {
      ...
      const idx = history.findIndex(
        (m) => m && m.role === 'assistant' && String(m.message_id || '').trim() === mid
      );
      if (idx < 0) return { ok: false, erro: 'mensagem_nao_encontrada' };

      const existing = history[idx];
      if (existing?.media_stored === true && String(existing?.storageFileId || existing?.storage_file_id || '').trim()) {
        return { ok: true, skipped: true };
      }
      if (opts.onlyIfPlaceholder && !isOutboundPhoneMediaPlaceholderContent(existing?.content)) {
        return { ok: true, skipped: true };
      }

      const nextMsg = { ...existing, ...patch };
      if (patch.type) nextMsg.type = String(patch.type).trim().toLowerCase();
      const nextHistory = history.slice();
      nextHistory[idx] = nextMsg;
      const nowIso = new Date().toISOString();
      const payload = {
        ...conversationMessagesStoragePayload(nextHistory, {
          academyId: current.academy_id,
          phoneNumber: current.phone_number,
        }),
        updated_at: nowIso,
        ...lastMessageMetaPayload(nextHistory),
      };
