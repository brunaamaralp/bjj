...
} from '../../src/lib/onboardingChecklist.js';
import { provisionDefaultTaskTemplates } from './taskTemplateStore.js';
import { defaultAcquirerFees } from '../../src/lib/acquirerFees.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const onboardingChecklistStr = serializeOnboardingChecklistForDb(DEFAULT_ONBOARDING_CHECKLIST);
    const aiName = String(body.ai_name || '').trim().slice(0, 80);
    if (!aiName) {
      return res.status(400).json({ sucesso: false, erro: 'ai_name \u00e9 obrigat\u00f3rio' });
    }
    const vertical = String(body.vertical || '').trim() === 'physio' ? 'physio' : 'fitness';
    const legalTermsVersion = String(body.legal_terms_version || '').trim();
    const legalPrivacyVersion = String(body.legal_privacy_version || '').trim();
    const legalAcceptedAt = String(body.legal_accepted_at || '').trim();
    ...
    const payload = {
      ...
      modules: JSON.stringify({ sales: true, inventory: false, finance: false }),
      quickTimes: [],
      financeConfig: JSON.stringify(defaultFinance),
      onboardingChecklist: onboardingChecklistStr,
      ...
      vertical,
      ...(legalTermsVersion ? { legal_terms_version: legalTermsVersion } : {}),
      ...(legalPrivacyVersion ? { legal_privacy_version: legalPrivacyVersion } : {}),
      ...(legalAcceptedAt ? { legal_accepted_at: legalAcceptedAt } : {}),
    };
    ...
    try {
      await provisionDefaultTaskTemplates(databases, DB_ID, doc.$id);
    } catch (tplErr) {
      console.warn('[academies/create] task templates:', tplErr?.message || tplErr);
    }
