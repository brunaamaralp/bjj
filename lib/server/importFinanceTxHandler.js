...
const IMPORT_FIELDS = ['date', 'amount', 'direction', 'category', 'note', 'method', 'competence_month'];
...
- amount (obrigatório): valor monetário (positivo ou negativo)
...
function looksLikeNativeFinanceTxHeaders(headers = []) {
  ...
  const amountCol = pickHeader(headers, ['valor', 'amount', 'value', 'quantia', 'total', 'vlr']);
...
function nativeMappingFromHeaders(headers = []) {
  ...
  mapping.amount = pickHeader(headers, ['valor', 'amount', 'value', 'quantia', 'total', 'vlr']);
