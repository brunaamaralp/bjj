...
import { parseAutomationsConfig, parsePendingAutomations, shouldSkipPendingAutomationResend, markPendingAutomationSent } from '../automationCore.js';
  ...
  while (pageIndex < maxPages) {
    ...
    for (const doc of docs) {
      ...
      for (let i = 0; i < nextPending.length; i += 1) {
        ...
        if (shouldSkipPendingAutomationResend(item, now)) {
        ...
        sent += 1;
        nextPending[i] = markPendingAutomationSent(item);
        changed = true;
      }
      if (changed) {
        try {
