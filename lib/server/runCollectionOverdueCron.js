...
function isActiveStudentLead(doc) {
  if (!isStudentLead(doc)) return false;
...
async function listAcademyLeads(databases, dbId, academyId) {
  if (!LEADS_COL) return [];
  const PAGE = 100;
  let all = [];
  let cursor = null;
  for (;;) {
    const queries = [Query.equal('academy_id', academyId), Query.limit(PAGE)];
    if (cursor) queries.push(Query.cursorAfter(cursor));
    const res = await databases.listDocuments(dbId, LEADS_COL, queries);
    const batch = res.documents || [];
    all = all.concat(batch);
    if (batch.length < PAGE) break;
    cursor = batch[batch.length - 1].$id;
...
async function processAcademy(databases, dbId, academyDoc) {
  ...
  const [leadsRaw, payments, tasks] = await Promise.all([
    listAcademyLeads(databases, dbId, academyId),
  ...
  for (const doc of leadsRaw) {
    ...
    if (overdue) {
      ...
      if (labelId && !labelIds.includes(labelId)) {
        ...
        await databases.updateDocument(dbId, LEADS_COL, student.id, { label_ids: labelIds });
        labelsApplied += 1;
      }
      ...
      if (labelId && labelIds.includes(labelId)) {
        ...
        if (next.length !== labelIds.length) {
          await databases.updateDocument(dbId, LEADS_COL, student.id, { label_ids: next });
          labelsRemoved += 1;
        }
      }
    }
