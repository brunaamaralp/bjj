...
export async function sendAutomationTemplateCron({
  ...
  if (!out?.ok) {
    if (out?.skipped === 'no_recent_interaction') {
      return { ok: false, skipped: 'no_recent_interaction' };
    }
    ...
    void createInternalNotification({
      ...
      action_url: phoneDisplay ? `/inbox?phone=${encodeURIComponent(phoneDisplay)}` : '/automacoes?tab=gatilhos',
      severity: 'high',
      phone: phoneDisplay,
      lead_id: leadId,
  ...
  if (academyId && leadId) {
    ...
    const payload = {
      ...
      sentAt,
      channel: 'cron',
      scheduledDate: String(leadDoc?.scheduledDate || '').slice(0, 10),
    };
    ...
    if (automationKey === 'followup_d1_attended' || automationKey === 'presence_confirmed') {
      await addLeadEventServer({
        academyId,
        leadId,
        type: 'followup_contact',
        text: 'Contato de retorno (automação)',
        at: sentAt,
        createdBy: 'automation',
        payloadJson: {
          source: 'automation',
