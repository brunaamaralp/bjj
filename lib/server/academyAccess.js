...
async function resolveAcademyAccess(academyId, me) {
...
export async function ensureAcademyOwnerOrAdmin(req, res, me) {
  ...
  if (teamId && userId) {
    try {
      ...
      if (roles.some((r) => teamRoleIsAdminOrOwner(r))) return access;
    } catch (e) {
...
/** Papéis de time equivalentes a gestor (client + Appwrite). */
function teamRoleIsAdminOrOwner(role) {
...
export async function isAcademyOwnerOrAdminUser(doc, me) {
  const ownerId = String(doc?.ownerId || '').trim();
  const userId = String((me && typeof me === 'object' ? me.$id : me) || '').trim();
  ...
  if (teamId && userId) {
    try {
      const m = await fetchTeamMembership(teamId, userId);
      const roles = Array.isArray(m?.roles) ? m.roles : [];
      if (roles.some((r) => teamRoleIsAdminOrOwner(r))) return true;
    } catch {
      void 0;
    }
  }
