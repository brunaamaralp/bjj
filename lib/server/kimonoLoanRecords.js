...
export async function reconcileOrphanRentalMoves(databases, dbId, stockItemsCol, academyId) {
  ...
  let created = 0;
  try {
    const [rentalKindRes, rentalTipoRes] = await Promise.all([
      databases.listDocuments(dbId, STOCK_MOVES_COL, [
        Query.equal('academy_id', [academyId]),
        Query.equal('movement_kind', ['rental']),
    ...
    const seen = new Set();
    const moves = [];
    for (const doc of [...(rentalKindRes.documents || []), ...(rentalTipoRes.documents || [])]) {
