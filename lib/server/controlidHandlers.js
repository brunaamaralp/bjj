...
import { matchBookingForCheckin, applyBookingCheckinMatch } from './bookingAttendanceMatch.js';
...
async function processAccessEvent(academyId, event, config = {}) {
  ...
  if (ATTENDANCE_COL) {
    try {
      ...
      let attendanceId = existing.documents?.[0]?.$id || '';
      if (existing.total === 0) {
      ...
      if (attendanceId) {
        try {
          const academy = await loadAcademy(academyId);
          const checkedInAtIso = new Date(ts * 1000).toISOString();
          const match = await matchBookingForCheckin(databases, DB_ID, {
            academyId,
          ...
          if (match) {
            await applyBookingCheckinMatch(databases, DB_ID, {
              ...
              checkedInAtIso,
              matchType: 'catraca',
              deviceLogId: String(log.id),
            });
          }
        } catch (matchErr) {
          console.warn('[controlid_monitor] booking match', matchErr?.message);
        }
      }
    } catch (e) {
