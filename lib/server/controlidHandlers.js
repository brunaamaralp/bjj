...
function safeControlIdStatusFromSettings(settings) {
  ...
  return {
    ...
    device_ip: cfg.ip || '',
    last_sync: String(raw.last_sync || raw.lastSync || '').trim(),
...
export async function controlidAttendanceHandler(req, res) {
  ...
  try {
    ...
    return json(res, 500, { sucesso: false, erro: e?.message || 'Erro ao buscar presenças' });
  }
}
...
export async function controlidSyncAllHandler(req, res) {
  ...
  try {
    ...
    return json(res, 200, { sucesso: false, erro: e?.message || 'Catraca não configurada' });
  }
  ...
  return json(res, 200, { sucesso: true, synced, failed, skipped, errors });
}
