 ...
 * @param {{ academyId: string, status?: string, assignedTo?: string, leadId?: string, overdue?: boolean, dueToday?: boolean, cursor?: string, pageLimit?: number, todayYmd?: string }} params
 */
export function buildTasksListQueries(params) {
  ...
  const dueToday = Boolean(params.dueToday);
  ...
  if (overdue) {
    ...
    queries.push(Query.equal('due_date', [todayYmd]));
