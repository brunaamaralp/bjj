...
import { assertAiModuleEnabled, AiFeatureDisabledError } from './aiFeaturePolicy.js';
import { buildLeadFieldsFromClassification } from '../agentClassificationFields.js';
import { Client, Databases, ID, Permission, Query, Role, Account, Teams } from 'node-appwrite';
import handleInventoryAdjustAgent from './inventoryAdjustAgent.js';
import handleInventoryQueryAgent from './inventoryReportAgent.js';
...
import { logTokenUsage, recordAgentRespondLatency } from './agentRespondMetrics.js';
import { assertHumanHandoffEnvOnBoot } from '../constants.js';
...
import { conversationMessagesStoragePayload } from './conversationMessages.js';
import { runAgentActions } from './agentActionExecutor.js';
import { getAiActionsPolicy, isConversationTimelineEnabled } from './agentActionPolicy.js';
import {
  LEAD_HISTORY_SUMMARY_SYSTEM,
  buildMessagesOnlySummaryContext,
...
async function ensureAcademyAccess(req, res, me) {
  ...
  try {
    ...
    res.status(500).json({ sucesso: false, erro: apiErro(e, 'action') });
    return null;
  }
}
...
async function generateSummary({ previousSummaryText, recentMessages, academy_id }) {
  const context = buildMessagesOnlySummaryContext(recentMessages);
  const userContent = formatSummaryUserPrompt({ context, previousText: previousSummaryText });
  ...
  return parseSummaryGenerationResponse(raw).summary;
}
  ...
  try {
    ...
    if (!resp.ok) {
      if (CLAUDE_RETRYABLE_HTTP_STATUS.includes(resp.status) && attempt < CLAUDE_MAX_RETRIES) {
        ...
        logStructured('claude_http_retry', {
          academy_id,
          error: `http_${resp.status}_retry_${attempt + 1}`,
        });
    ...
    const text = parts
      ...
      .join('\n')
      .trim();
    recordAgentRespondLatency(academy_id, Date.now() - started);
    return text;
  } catch (e) {
    clearTimeout(timeoutId);
    ...
    if (name === 'AbortError' && attempt < CLAUDE_MAX_RETRIES) {
      const delay = CLAUDE_RETRY_DELAY_MS * 2 ** attempt;
      logStructured('claude_retry', {
        academy_id,
        error: `timeout_retry_${attempt + 1}`,
      });
      await new Promise((r) => setTimeout(r, delay));
      return callClaude({ system, messages, maxTokens, temperature, academy_id }, attempt + 1);
    }
    if (name === 'AbortError') {
      recordAgentRespondLatency(academy_id, Date.now() - started, { timedOut: true });
      logStructured('claude_timeout', {
        academy_id,
        error: `timeout_${CLAUDE_TIMEOUT_MS}ms`,
      });
...
async function updateLeadNotesFromClassification(leadDoc, classificacao) {
  ...
  const patch = buildLeadFieldsFromClassification(classificacao);
  if (Object.keys(patch).length === 0) return;
  await databases.updateDocument(DB_ID, LEADS_COL, leadDoc.$id, patch);
}
...
export default async function handler(req, res) {
  const adjustMode =
    String(req.query?.mode || '').trim() === 'inventory_adjust' ||
    String(req.body?.mode || '').trim() === 'inventory_adjust';
  if (adjustMode && req.method === 'POST') {
    return handleInventoryAdjustAgent(req, res, databases);
  }
  const queryMode =
    String(req.query?.mode || '').trim() === 'inventory_query' ||
    String(req.body?.mode || '').trim() === 'inventory_query';
  if (queryMode && req.method === 'POST') {
    return handleInventoryQueryAgent(req, res, databases);
  }
  if (req.method !== 'POST') {
  ...
  if (isInternal) {
    try {
      ...
      if (e instanceof AiFeatureDisabledError) {
        return res.status(403).json({ sucesso: false, erro: e.message, error: e.code });
      }
      throw e;
    }
  ...
  try {
    let leadDoc = null;
    try {
      leadDoc = await findLeadByPhone(databases, phone, academyId);
    } catch {
      leadDoc = null;
    }
    ...
    if (!leadDoc && doc?.$id) {
      try {
        ...
        if (ensured?.leadDoc) leadDoc = ensured.leadDoc;
      } catch {
        void 0;
      }
    }
    ...
    if (humanHandoffIsActive(handoffUntilRaw)) {
      logStructured('agent_respond_skipped', {
    ...
    const actionsEnabled = getAiActionsPolicy(academyDoc).enabled;
    const timelineEnabled = isConversationTimelineEnabled(academyDoc);
    if (actionsEnabled || timelineEnabled) {
