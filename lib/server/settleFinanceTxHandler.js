...
export default async function settleFinanceTxHandler(req, res) {
  ...
  try {
    ...
    if (!String(doc.competence_month || '').trim()) {
      patch.competence_month = competenceMonthFromIso(now);
    }
    const updated = await databases.updateDocument(DB_ID, FINANCIAL_TX_COL, String(transactionId), patch);
