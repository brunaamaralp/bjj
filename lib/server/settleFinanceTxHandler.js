import { ensureAuth, ensureAcademyAccess, isAcademyOwnerOrAdminUser, databases, DB_ID } from './academyAccess.js';
...
export default async function settleFinanceTxHandler(req, res) {
  ...
  const mappedPrev = mapFinanceTxDoc(doc);
  const prevType = String(doc.type || '').toLowerCase();
  const isOutflow =
    (mappedPrev && txDirection(mappedPrev) === 'out') || isExpenseType(prevType);
  if (isOutflow && !(await isAcademyOwnerOrAdminUser(academyDoc, me))) {
    return res.status(403).json({ error: 'forbidden' });
  }
