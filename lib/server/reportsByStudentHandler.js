...
import { isBundleChildPayment } from '../../src/lib/paymentCategories.js';
...
async function listAllSaleItemsForSales(saleIds) {
  if (!saleIds.length || !SALE_ITEMS_COL) return [];
  const CHUNK = 25;
  const chunks = [];
...
function mapSaleStatus(st) {
  const s = String(st || '').toLowerCase();
  if (s === 'concluida') return 'paid';
  if (s === 'pendente' || s === 'rascunho') return 'pending';
