...
function snapshotKey(academyId, from, to, filters, chartMode) {
...
export async function loadReportSnapshot(academyId, from, to, filters, chartMode) {
  const col = SNAPSHOT_COL();
  if (!col || !DB_ID) return null;
  const key = snapshotKey(academyId, from, to, filters, chartMode);
  try {
...
export async function saveReportSnapshot(academyId, from, to, filters, chartMode, payload) {
  const col = SNAPSHOT_COL();
  if (!col || !DB_ID) return null;
  const key = snapshotKey(academyId, from, to, filters, chartMode);
