...
const SUMMARY_SYSTEM = `Você resume o histórico de um lead/aluno de academia de Jiu-Jitsu para a recepção.
...
async function loadWaMessages(academyId, phone, leadId) {
  if (!CONVERSATIONS_COL) return [];
  const doc = await findConversationDoc(phone, academyId, {
    leadId: String(leadId || '').trim(),
  });
  return resolveConversationMessagesFromDoc(doc);
}
...
function buildContextBlock({ lead, messages, events, academyName, daysAgo, nextAction, forSummary = false }) {
  const lines = [
    `Lead: ${lead.name || '—'}`,
    `Status: ${lead.status || '—'}`,
  ...
  if (events.length) {
    ...
    for (const e of events.slice(0, forSummary ? 15 : 8)) {
  ...
  if (forSummary && !messages.length && !events.length) {
...
export default async function followupCopilotHandler(req, res) {
  ...
  const [messages, events] = await Promise.all([
    loadWaMessages(academyId, lead.phone, leadId),
