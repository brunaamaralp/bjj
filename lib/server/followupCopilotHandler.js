...
async function callClaude({ system, userContent, maxTokens = 700, temperature = 0.1 }) {
...
export default async function followupCopilotHandler(req, res) {
  ...
  try {
    ...
    const raw = await callClaude({ system: DRAFT_SYSTEM, userContent, maxTokens: 500, temperature: 0.35 });
