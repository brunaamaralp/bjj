...
async function handlePublicGet(req, res) {
  ...
  const classes = await listAcademyClassDocs(databases, resolved.academyId);
  const form = buildPublicEnrollmentFormConfig(resolved.academy, { classes });
...
async function handlePublicPost(req, res) {
  ...
  const classes = await listAcademyClassDocs(databases, resolved.academyId);
  const formConfig = buildPublicEnrollmentFormConfig(resolved.academy, { classes });
  ...
  try {
    ...
    if (code === 'birth_date_required' || code === 'birth_date_invalid') {
      return json(res, 400, {
        sucesso: false,
        erro: code,
        message: e?.message || 'Informe a data de nascimento do aluno.',
      });
    }
...
async function handleConfigPost(req, res) {
  ...
  const askBeltPatch = body.askBelt !== undefined ? body.askBelt === true : undefined;
  ...
  try {
    ...
    const merged = mergePublicEnrollmentIntoSettings(doc.settings, {
      ...
      ...(askBeltPatch !== undefined ? { askBelt: askBeltPatch } : {}),
    });
