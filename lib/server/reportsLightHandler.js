...
import { aggregateOperationalSummary, aggregateRevenueBreakdown, aggregateOperationalWeeklySeries } from './financeTxAggregate.js';
...
export async function financeSummary(academyId, from, to, regime) {
  ...
  const weeklySeries = aggregateOperationalWeeklySeries(documents, from, to);
...
/**
 * Busca itens de venda para um conjunto de IDs em batch (sem N+1).
 * Appwrite suporta Query.equal('field', array) como IN-query.
 * Batches de até 25 IDs para respeitar limites do parser de query.
 */
async function fetchSaleItemsBatch(saleIds) {
...
export async function salesSummary(academyId, from, to) {
  ...
  if (SALE_ITEMS_COL && concludedSaleIds.length) {
    const allItems = await fetchSaleItemsBatch(concludedSaleIds);
