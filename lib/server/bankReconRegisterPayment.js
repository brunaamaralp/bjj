...
import { isStorageCreditMethod } from '../../src/lib/paymentMethods.js';
...
function normalizePaymentInstallments(method, installments) {
...
function buildPaidPaymentPayload(data, financeConfig, studentDoc, me) {
  ...
  const installments = normalizePaymentInstallments(method, data.installments);
  const amount = roundMoney(data.amount);
  const student = {
    plan: studentDoc?.plan,
    dueDay: studentDoc?.due_day ?? studentDoc?.dueDay,
  };
  let expected = expectedAmountWithCardFee(student, financeConfig, method, installments, data);
  ...
  return {
    ...
    ...(data.capture_method_id
      ? {
          capture_method_id: String(data.capture_method_id).slice(0, 64),
          capture_method_name: String(data.capture_method_name || '').slice(0, 128),
          fee_receiver_id: String(data.fee_receiver_id || '').slice(0, 64),
        }
      : {}),
    ...(data.card_brand ? { card_brand: String(data.card_brand).trim().slice(0, 64) } : {}),
  };
}
...
export async function registerReconPayment({
  ...
  const paymentData = {
    ...
    paid_at: paidAt,
    capture_method_id: body.capture_method_id,
    capture_method_name: body.capture_method_name,
    fee_receiver_id: body.fee_receiver_id,
