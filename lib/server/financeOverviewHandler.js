...
import { addDaysYmd } from '../../src/lib/financeForecastCore.js';
...
export default async function financeOverviewHandler(req, res) {
  ...
  try {
    ...
    const [forecastFull, contractsAwaitingCount, payablesInputs] = await Promise.all([
      includeForecast
      ...
      canIncludePayables ? loadPayablesInputs(academyId) : Promise.resolve(null),
    ]);
