...
async function computeOpeningBalance(academyId, todayEndIso) {
  ...
  try {
    ...
    if (!msg.includes('Unknown attribute') && !msg.toLowerCase().includes('settledat')) throw e;
    docs = await fetchAllPages(FINANCIAL_TX_COL, [
      Query.equal('academyId', academyId),
      Query.equal('status', ['settled']),
    ]);
...
async function loadStudentsForLeads(academyId, leadIds) {
  const names = new Map();
  ...
  if (ids.length > 15) {
    try {
      const all = await fetchAllPages(STUDENTS_COL, [Query.equal('academyId', academyId)]);
...
async function listOpenPayments(academyId, fromYmd, toYmd) {
  ...
  if (!STUDENTS_COL) return [...byDocId.values()];

  let students = [];
  try {
    students = await fetchAllPages(STUDENTS_COL, [Query.equal('academyId', academyId)]);
  } catch {
    students = [];
  }
  if (!students.length) return [...byDocId.values()];
  ...
  return [...byDocId.values()].filter((p) => {
...
export async function buildFinanceForecast(academyId, fromYmd, toYmd, { financeConfig = {} } = {}) {
  ...
  const payments = await listOpenPayments(academyId, from, to);
...
export default async function financeForecastHandler(req, res) {
  ...
  try {
    ...
    return json(res, 500, { ok: false, error: 'forecast_failed', detail: e?.message });
  }
}
