...
describe('salesCancelHandler', () => {
  ...
  it('em venda cancelada sem movimento de retorno, restaura estoque via snapshot', async () => {
    mocks.getDocument.mockResolvedValue({
      $id: 'sale-done',
      academyId: 'acad-1',
      status: 'cancelada',
      cancel_motivo: 'Erro',
      cancelada_em: '2026-07-13T00:00:00.000Z',
      itens_snapshot_json: JSON.stringify([
        { item_estoque_id: 'legacy-stock-id', quantidade: 1, preco_unitario: 50 },
      ]),
    });
    // 1) moves vazios 2) sale_items vazios
    mocks.listDocuments.mockResolvedValue({ documents: [] });
    mocks.resolveStockDocument.mockResolvedValue({
      collection: 'variants-col',
      doc: {
    ...
    mocks.createStockMoveDocument.mockResolvedValue({ $id: 'move-repair' });

    const res = mockRes();
    await salesCancelHandler(
      {
        method: 'PATCH',
        body: { id: 'sale-done', action: 'cancelar', motivo: 'reparo' },
      },
      res
    );
    ...
    expect(res.body.items_source).toBe('snapshot');
    expect(mocks.createStockMoveDocument).toHaveBeenCalled();
    expect(mocks.cancelSaleFinancials).not.toHaveBeenCalled();
  });
});
