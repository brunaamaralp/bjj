...
export function financeUserNoteFromStored(note) {
  return String(note || '').replace(/^@cat:[^\n]+\n?/, '').trim();
}

export function financeNoteForStorage(categoryLabel, userNote) {
  const cat = String(categoryLabel || '').trim();
  const body = financeUserNoteFromStored(userNote);
  if (!cat) return body.slice(0, 2000);
  const prefix = `${FINANCE_CAT_NOTE_PREFIX}${cat}\n`;
...
/** Atributos que o Appwrite pode não ter; inferidos em mapFinanceTxDoc via type/settledAt/note. */
const APWRITE_SKIP_ATTRS = ['direction'];

/** Payload enviado ao Appwrite (sem campos derivados). */
export function financeTxDocumentForAppwrite(payload) {
  const doc = { ...payload };
  for (const key of APWRITE_SKIP_ATTRS) delete doc[key];
  return doc;
}

/** Remove atributos opcionais ausentes no schema Appwrite. */
export function stripUnknownFinanceTxAttrs(payload, errorMessage = '') {
  const lean = { ...payload };
  const msg = String(errorMessage || '');
  if (!/unknown attribute/i.test(msg)) return lean;

  const category = lean.category;
  const userNote = financeUserNoteFromStored(lean.note);
  for (const key of OPTIONAL_FINANCIAL_TX_ATTRS) {
    delete lean[key];
  }
  if (category) {
    lean.note = financeNoteForStorage(category, userNote);
  }
  return lean;
}
