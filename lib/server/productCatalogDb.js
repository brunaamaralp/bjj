...
import { recordCadastroInicialStockMove } from './stockMoveFields.js';
...
async function listDocumentsForAcademy(databases, dbId, collectionId, academyId) {
  ...
  try {
    ...
    console.warn(
      `[listCatalog] academy_id filter failed on ${collectionId}:`,
      err?.message || err
    );
    try {
      const list = await databases.listDocuments(dbId, collectionId, [Query.limit(500)]);
      return (list.documents || []).filter(
        (d) => !d.academy_id || String(d.academy_id) === academyId
      );
    } catch (err2) {
      console.warn(`[listCatalog] list failed on ${collectionId}:`, err2?.message || err2);
      return [];
    }
  }
}
...
async function listParents(databases, dbId, academyId) {
  const docs = await listDocumentsForAcademy(databases, dbId, PRODUCTS_COL, academyId);
  return docs.map(mapParentProductDoc);
}

/** Produto pai existente com o mesmo nome (normalizado) na academia. */
export async function findParentDocByName(databases, dbId, academyId, name) {
  const needle = normalizeParentNameKey(name);
  if (!needle || !PRODUCTS_COL) return null;
  const docs = await listDocumentsForAcademy(databases, dbId, PRODUCTS_COL, academyId);
  return (
    docs.find((d) => normalizeParentNameKey(d.name || d.nome) === needle) || null
  );
}

async function listVariantsForAcademy(databases, dbId, academyId) {
  return listDocumentsForAcademy(databases, dbId, PRODUCT_VARIANTS_COL, academyId);
}
...
export async function resolveStockDocument(databases, dbId, stockItemsCol, itemId) {
  ...
  if (isParentVariantCatalogEnabled()) {
    ...
    try {
      const legacyMatch = await databases.listDocuments(dbId, PRODUCT_VARIANTS_COL, [
        Query.equal('legacy_stock_item_id', id),
        Query.limit(2),
      ]);
      const docs = legacyMatch.documents || [];
      if (docs.length === 1) {
...
export async function listCatalog(databases, dbId, stockItemsCol, academyId) {
  ...
  try {
    return await listParentVariantCatalog(databases, dbId, stockItemsCol, academyId);
  } catch (err) {
    console.error('[listCatalog] parent_variant failed, fallback legacy:', err?.message || err);
    const legacy = await listLegacyStockItems(databases, dbId, stockItemsCol, academyId);
    const variants = legacy.map(mapStockProductDoc);
    return { catalog_mode: 'legacy', products: [], variants, needs_migration: false };
  }
}

async function listParentVariantCatalog(databases, dbId, stockItemsCol, academyId) {
  // Parents, variants e legacy em paralelo — legacy listado uma vez (needs_migration + imagens).
  const [parents, variantDocs, legacyDocs] = await Promise.all([
    listParents(databases, dbId, academyId),
    listVariantsForAcademy(databases, dbId, academyId),
    stockItemsCol ? listLegacyStockItems(databases, dbId, stockItemsCol, academyId) : Promise.resolve([]),
  ]);
  const parentById = new Map(parents.map((p) => [p.id, p]));
  const unmigrated = countUnmigratedFromLegacyDocs(legacyDocs);
  ...
  if (orphanStubs.length) {
    ...
    products = mergeCatalogParentRowsByName(products, orphanRows);
  }

  const catalogFromVariants = products.length > 0 || variants.length > 0;

  // Só mescla STOCK_ITEMS legado quando o catálogo pai/variante ainda está vazio.
  if (!catalogFromVariants && legacyDocs.length) {
    const linkedLegacyIds = new Set(
      (variantDocs || [])
        .map((d) => String(d.legacy_stock_item_id || '').trim())
        .filter(Boolean)
    );
    const legacyToAppend = legacyDocs.filter((d) => {
      const id = String(d.$id || '').trim();
      return id && !linkedLegacyIds.has(id);
    });
    if (legacyToAppend.length) {
      ({ products, variants } = appendUnmigratedLegacyCatalog(products, variants, legacyToAppend));
      products = mergeCatalogParentRowsByName(products);
    }
  }
...
export function countUnmigratedFromLegacyDocs(legacyDocs) {
  return (legacyDocs || []).filter((d) => d.migrated !== true).length;
}

export async function countUnmigratedLegacy(databases, dbId, stockItemsCol, academyId) {
  const legacy = await listLegacyStockItems(databases, dbId, stockItemsCol, academyId);
  return countUnmigratedFromLegacyDocs(legacy);
}

async function variantHasStockMoves(databases, dbId, stockMovesCol, academyId, variantId) {
  if (!stockMovesCol || !variantId) return false;
  const res = await databases.listDocuments(dbId, stockMovesCol, [
    Query.equal('academy_id', academyId),
    Query.equal('item_estoque_id', variantId),
    Query.limit(1),
  ]);
  return (res.total || 0) > 0;
}

async function remapStockMovesItemId(databases, dbId, stockMovesCol, academyId, fromId, toId) {
  const from = String(fromId || '').trim();
  const to = String(toId || '').trim();
  if (!stockMovesCol || !from || !to || from === to) return 0;
  ...
  for (;;) {
    const queries = [
      Query.equal('academy_id', academyId),
      Query.equal('item_estoque_id', from),
      Query.limit(100),
    ];
    ...
    for (const move of batch) {
      await databases.updateDocument(dbId, stockMovesCol, move.$id, { item_estoque_id: to });
      remapped += 1;
    }
...
async function ensureCadastroInicialMove(databases, { dbId, stockMovesCol, academyId, variantId, quantidade, usuarioId }) {
  const qty = Math.trunc(Math.max(0, Number(quantidade) || 0));
  if (!stockMovesCol || qty <= 0) return { ok: true, skipped: true };
  const hasMoves = await variantHasStockMoves(databases, dbId, stockMovesCol, academyId, variantId);
  if (hasMoves) return { ok: true, skipped: true, reason: 'has_moves' };
  return recordCadastroInicialStockMove(databases, {
    dbId,
    stockMovesCol,
    academyId,
    itemEstoqueId: variantId,
    quantidade: qty,
    usuarioId,
    quantityBefore: 0,
  });
...
export async function migrateLegacyStockItems(databases, dbId, stockItemsCol, stockMovesCol, academyId, me) {
  ...
  for (const items of groups.values()) {
    ...
    let parentDoc = await findParentDocByName(databases, dbId, academyId, baseName);
    if (!parentDoc) {
    ...
    const existingOnParent = await listVariantsByProduct(databases, dbId, parentDoc.$id);
    const migratedComboSeen = new Set(
      existingOnParent.map((v) => variantComboKey(v.size, v.color))
    );
    for (const { doc, mapped } of items) {
      ...
      if (stockMovesCol) {
        try {
          movesRemapped += await remapStockMovesItemId(
            databases,
            dbId,
            stockMovesCol,
            academyId,
          ...
          const cadResult = await ensureCadastroInicialMove(databases, {
            dbId,
            stockMovesCol,
            academyId,
            variantId: variantDoc.$id,
            quantidade: pools.current_quantity,
            usuarioId: me?.$id,
          });
          if (cadResult.ok && cadResult.movimento_id) cadastroMovesCreated += 1;
          if (!cadResult.ok && !cadResult.skipped) {
            stockMoveWarnings.push({
              legacy_id: doc.$id,
              variant_id: variantDoc.$id,
              error: cadResult.error || 'cadastro_inicial_failed',
            });
          }
        } catch (e) {
          stockMoveWarnings.push({
      ...
      } else if (pools.current_quantity > 0) {
        stockMoveWarnings.push({
          legacy_id: doc.$id,
          variant_id: variantDoc.$id,
          error: 'stock_moves_not_configured',
        });
      }
  ...
  return {
    ...
    moves_remapped: movesRemapped,
    cadastro_moves_created: cadastroMovesCreated,
    stock_move_warnings: stockMoveWarnings,
    skipped: 0,
  };
}
...
export async function createProductWithVariants(
  ...
  if (reuseExistingParent) {
    parentDoc = await findParentDocByName(databases, dbId, academyId, built.payload.name);
  }
  if (!parentDoc) {
    parentDoc = await createParentDocument(databases, dbId, built.payload);
    parentCreated = true;
  }
  const parent = mapParentProductDoc(parentDoc);

  const existingVariants = await listVariantsByProduct(databases, dbId, parentDoc.$id);
  const existingComboKeys = new Set(
    existingVariants.map((d) => variantComboKey(d.size, d.color))
  );

  const createdVariants = [];

  for (const v of variants) {
    const comboKey = variantComboKey(v.size, v.color);
    ...
    if (pools.current_quantity > 0) {
      if (!stockMovesCol) {
        await databases.deleteDocument(dbId, PRODUCT_VARIANTS_COL, variantDoc.$id).catch(() => {});
        return {
          error: 'Movimentações de estoque não configuradas (STOCK_MOVES_COL)',
          status: 503,
        };
      }
      const moveResult = await recordCadastroInicialStockMove(databases, {
        dbId,
        stockMovesCol,
        academyId,
      ...
      if (!moveResult.ok && !moveResult.skipped) {
        await databases.deleteDocument(dbId, PRODUCT_VARIANTS_COL, variantDoc.$id).catch(() => {});
        return {
          error: moveResult.error || 'Erro ao registrar saldo inicial no histórico de estoque',
          status: 500,
        };
      }
    }
  ...
  const allVariants = [
    ...existingVariants.map((d) => mapVariantDoc(d, parent)),
    ...createdVariants,
  ];
  ...
  return {
    product: {
      ...
      total_quantity: allVariants.reduce((n, x) => n + x.current_quantity, 0),
      variant_count: allVariants.length,
      lifecycle: allVariants.some((x) => x.lifecycle === 'ativo') ? 'ativo' : 'sem_estoque',
    },
    variants: createdVariants,
    parent_created: parentCreated,
    variants_skipped: variants.length - createdVariants.length,
  };
}
  ...
  for (const row of activeRows) {
    ...
    if (!row.id) {
      ...
      try {
        ...
        if (pools.current_quantity > 0 && !stockMovesCol) {
          errors.push({
            variant_id: null,
            label,
            code: 'stock_moves_not_configured',
            message: 'Movimentações de estoque não configuradas (STOCK_MOVES_COL)',
          });
          continue;
        }
        ...
        if (pools.current_quantity > 0 && stockMovesCol) {
          const move = await recordCadastroInicialStockMove(databases, {
            dbId,
            stockMovesCol,
            academyId,
            itemEstoqueId: variantDoc.$id,
