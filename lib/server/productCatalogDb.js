...
export async function listCatalog(databases, dbId, stockItemsCol, academyId) {
  ...
  const variantIds = new Set(variants.map((v) => String(v.id || '').trim()).filter(Boolean));
  const catalogFromVariants = products.length > 0 || variants.length > 0;

  const legacyToAppend = legacyDocs.filter((d) => {
    const id = String(d.$id || '').trim();
    if (!id) return false;
    if (linkedLegacyIds.has(id)) return false;
    if (variantIds.has(id)) return false;
    if (d.migrated === true) {
      // Legado migrado só reaparece se o catálogo pai/variante estiver vazio (migração incompleta).
      return !catalogFromVariants;
    }
    return true;
  });
