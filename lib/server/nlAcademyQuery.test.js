...
describe('inferAcademyQueryType', () => {
  ...
  it('detecta cooling_followups', () => {
    expect(inferAcademyQueryType('Quem está esfriando no retorno?')).toBe('cooling_followups');
  });

  it('detecta attended_not_contacted_yesterday', () => {
    expect(inferAcademyQueryType('Quem compareceu ontem e não foi contatado?')).toBe(
      'attended_not_contacted_yesterday'
    );
  });
});
...
describe('answerAcademyQuery', () => {
  ...
  it('payables_this_week exige owner/admin', async () => {
    const out = await answerAcademyQuery(mockDatabases, {
      academyId: 'acad-1',
      queryType: 'payables_this_week',
      isOwnerOrAdmin: false,
    });
    expect(out.count).toBe(0);
    expect(out.resposta).toMatch(/titular ou administrador/i);
  });
});
