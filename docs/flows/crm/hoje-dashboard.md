...
**Arquivos-chave:** `src/pages/Dashboard.jsx`, `src/components/recepcao/RecepcaoCatracaTab.jsx`, `src/components/recepcao/RecepcaoSchedulesGrid.jsx`, `src/components/recepcao/KimonoLoanPanel.jsx`, `src/components/dashboard/*`, `src/lib/recepcaoHubTabs.js`
...
1. **Comercial** (default, sem `?tab`) — hero com KPIs (hoje, follow-ups, tarefas, vendas*, matrículas), **agenda da semana** em destaque, **kimonos** (disponíveis + emprestados), follow-ups, tarefas de hoje, grade de horários.
...
| 11d | `/` | `KimonoLoanPanel` | Ver **Kimonos** | Resumo disponíveis/emprestados; chips por tamanho; lista de empréstimos ativos; **Emprestar** abre modal; alerta de atraso em seção recolhida |
...
11d. [ ] **Kimonos** — painel mostra totais, chips de peças disponíveis e lista de emprestados; **Emprestar** registra saída; **Devolver** encerra empréstimo; configuração de alerta fica recolhida
