...
const paymentMocks = vi.hoisted(() => ({
  ...
  createSessionJwt: vi.fn().mockResolvedValue('mock-jwt-token'),
}));

function mockAppwriteModule(overrides = {}) {
  return {
    databases: {
...
describe('Pagamentos de mensalidade', () => {
  beforeEach(() => {
    ...
    paymentMocks.createSessionJwt.mockResolvedValue('mock-jwt-token');
    ...
    vi.stubEnv('VITE_USE_STUDENT_PAYMENTS_API', 'false');
  });
  ...
  describe('createPayment', () => {
    it('faz upsert quando já existe pagamento plan no mês', async () => {
      ...
      vi.doMock('../lib/appwrite.js', () => mockAppwriteModule());
    ...
    it('lança erro quando lead_id e academy_id estão ausentes', async () => {
      vi.doMock('../lib/appwrite.js', () => mockAppwriteModule());
      const { createPayment } = await import('../lib/studentPayments.js');
      await expect(createPayment({})).rejects.toThrow('lead_id e academy_id são obrigatórios');
    });
    ...
    it('lança erro se PAYMENTS_COL não estiver configurado', async () => {
      ...
      ).rejects.toThrow('student_payments_collection_not_configured');
    });
    ...
    it('com espelho FINANCIAL_TX e writeback desligado, não chama updateDocument com financial_tx_id', async () => {
      vi.stubEnv('VITE_STUDENT_PAYMENT_WRITE_FINANCIAL_TX_REF', 'false');
      vi.resetModules();
      paymentMocks.listDocuments.mockResolvedValueOnce({ documents: [] });
      paymentMocks.createDocument
