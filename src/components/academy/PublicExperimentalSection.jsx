...
import {
  ...
  buildPublicExperimentalUrl,
} from '../../lib/publicExperimentalSettings';
import ConfirmDialog from '../shared/ConfirmDialog.jsx';

async function postExperimentalConfig(academyId, body) {
  ...
  const res = await fetch('/api/leads?route=public-experimental-config', {
    method: 'POST',
    headers: {
...
export default function PublicExperimentalSection({
  academyId,
  academy,
  setAcademy,
  canEdit,
  ...
  const cfg = readPublicExperimental(academy?.settings);
  ...
  const shareUrl = shareToken ? buildPublicExperimentalUrl(shareToken) : '';
  ...
  const syncFromServer = useCallback(
    async (nextEnabled, regenerate = false) => {
      ...
      setBusy(true);
      try {
        const data = await postExperimentalConfig(academyId, {
          enabled: nextEnabled,
          regenerate,
        });
        ...
        const merged = mergePublicExperimentalIntoSettings(doc.settings, {
          ...
          salt: readPublicExperimental(doc.settings).salt,
        });
        ...
        addToast({
          type: 'success',
          message: data.enabled ? 'Link de experimental ativado.' : 'Link de experimental desativado.',
        });
      } catch (e) {
  ...
  const handleToggle = () => {
    void syncFromServer(!enabled, false);
  };
  ...
  return (
    <div className="card" style={{ marginTop: embedded ? 16 : 24, padding: embedded ? 16 : undefined }}>
      {!embedded ? (
      ...
      <p className="text-small text-muted" style={{ lineHeight: 1.45, marginBottom: 14 }}>
        Compartilhe para o interessado agendar a experimental pelo celular. O contato entra no funil em{' '}
        <strong>Aula experimental</strong> com data e hora. Se já existir lead com o mesmo telefone, o horário é
        atualizado.
      </p>
      ...
      {canEdit ? (
        <>
          <label className="flex items-center gap-2" style={{ fontSize: 14, marginBottom: 12, cursor: 'pointer' }}>
            ...
            <span>Permitir agendamento pelo link público</span>
          </label>
