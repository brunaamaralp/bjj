 ...
 * @param {'default' | 'on-dark' | 'on-light' | 'white' | 'app'} [variant='default']
 * @param {string} [className]
 */
export default function NaviLogo({ size = 26, variant = 'default', className = '' }) {
  const isApp = variant === 'app';
  const onDark = isApp || variant === 'on-dark' || variant === 'white';
  const src = isApp
    ? NAVI_BRAND_ASSETS.appIcon
    : onDark
      ? NAVI_BRAND_ASSETS.iconDark
      : NAVI_BRAND_ASSETS.iconLight;
  ...
  return (
    <img
      ...
      width={size}
      height={size}
      className={`navi-logo${isApp ? ' navi-logo--app' : onDark ? ' navi-logo--on-dark' : ' navi-logo--on-light'}${className ? ` ${className}` : ''}`}
