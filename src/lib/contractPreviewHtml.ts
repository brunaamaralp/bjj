...
/** Destaca tokens {{variavel}} no HTML do editor visual. */
...
/** Remove spans de destaque antes de salvar o HTML canônico. */
...
/** Extrai estilos e corpo de HTML colado (documento completo ou fragmento). */
...
/** HTML preparado para o editor visual (contentEditable). */
...
/** Reincorpora edições do modo visual no HTML canônico (preserva estilos e estrutura). */
...
/** Monta documento HTML isolado para pré-visualização responsiva no iframe. */
export function buildContractPreviewDocument(html: string): string {
  const { body, styles } = extractContractPreviewParts(html);
...
<body>${highlightContractVariableTokens(body) || '<p><em>(documento vazio)</em>'}</body>
</html>`;
}
