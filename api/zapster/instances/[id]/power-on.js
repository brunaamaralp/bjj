...
export default async function handler(req, res) {
  ...
  try {
    const url = `${baseUrl()}/v1/wa/instances/${encodeURIComponent(idParam)}/power-on`;
    ...
    return res.status(resp.status || 500).json({ sucesso: false, erro: text || 'Falha ao ligar' });
  } catch (e) {
    return res.status(500).json({ sucesso: false, erro: e?.message || 'Erro ao ligar' });
  }
}
