...
export default async function handler(req, res) {
  ...
  try {
    const url = `${baseUrl()}/v1/wa/instances/${encodeURIComponent(idParam)}/power-off`;
    ...
    return res.status(resp.status || 500).json({ sucesso: false, erro: text || 'Falha ao desligar' });
  } catch (e) {
    return res.status(500).json({ sucesso: false, erro: e?.message || 'Erro ao desligar' });
  }
}
